...
if (isset($tokens['success']) && $tokens['success'] == '0' || isset($tokens['success']) && $tokens['success'] == 0) {
    ...
    foreach($permissions as $permission){
        array_push($arrPermission, $permission->id);
    }
    if ($is_owner == '1' || $w47 == '0'){
        $permissionQuery = "";
    } else {
        $permissionQuery = " AND d.id IN (" . implode(',', $arrPermission) . ") ";
    }
    ...
    if ($newDateFormat == 1) {
        ...
        if (mysqli_num_rows($fav) > 0) {
            ...
            while ($rowMenu = mysqli_fetch_assoc($fav)) {
                ...
                $rounding = $rowMenu['rounding'];
                $singlePrice = $rowMenu['harga_satuan'];
                $sku = $rowMenu['sku'];
                $category = $rowMenu['cat_name'];
                $categoryID = $rowMenu['cat_id'];
                $departmentID = $rowMenu['dept_id'];
                $departmentName = $rowMenu['dept_name'];
                ...
                $reportVar['dept_id'] = $departmentID;
                $reportVar['dept_name'] = $departmentName;
        ...
        if ($all !== "1") {
            $query = "SELECT detail_transaksi.variant, detail_transaksi.is_program, detail_transaksi.id_transaksi, detail_transaksi.harga, transaksi.rounding, menu.nama, menu.id AS id_menu, detail_transaksi.harga_satuan as harga_satuan, detail_transaksi.status AS menu_status, transaksi.status AS transc_status, detail_transaksi.qty, detail_transaksi.id AS det_transc_id, menu.hpp, menu.sku, c.name AS cat_name, c.id AS cat_id, c.is_consignment, transaksi.id_partner, d.id AS dept_id, d.name AS dept_name FROM menu join detail_transaksi on menu.id=detail_transaksi.id_menu join transaksi on detail_transaksi.id_transaksi = transaksi.id join partner on transaksi.id_partner=partner.id JOIN categories c ON c.id = menu.id_category LEFT JOIN departments d ON d.id = c.department_id WHERE partner.id= '$id' AND transaksi.status IN (1,2,3,4) ".$permissionQuery." AND DATE(transaksi.paid_date) BETWEEN '$dateFrom' AND '$dateTo' ORDER BY detail_transaksi.id_transaksi DESC";
        } else {
            $query = "SELECT detail_transaksi.variant, detail_transaksi.is_program, detail_transaksi.id_transaksi, detail_transaksi.harga, transaksi.rounding, menu.nama, menu.id AS id_menu, detail_transaksi.harga_satuan as harga_satuan, detail_transaksi.status AS menu_status, transaksi.status AS transc_status, detail_transaksi.qty, detail_transaksi.id AS det_transc_id, partner.id AS id_partner, menu.hpp, menu.sku, c.name AS cat_name, c.id AS cat_id, c.is_consignment, d.id AS dept_id, d.name AS dept_name FROM menu join detail_transaksi on menu.id=detail_transaksi.id_menu join transaksi on detail_transaksi.id_transaksi = transaksi.id join partner on transaksi.id_partner=partner.id JOIN categories c ON c.id = menu.id_category LEFT JOIN departments d ON d.id = c.department_id WHERE partner.id_master= '$idMaster' AND transaksi.status IN (1,2,3,4) ".$permissionQuery." AND DATE(transaksi.paid_date) BETWEEN '$dateFrom' AND '$dateTo' ORDER BY detail_transaksi.id_transaksi DESC";
        }
