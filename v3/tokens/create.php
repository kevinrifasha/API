...
foreach ($headers as $header => $value) {
    ...
    $tknDecrypt = json_decode($tokenizer->stringEncryption('decrypt',$token));
    ...
    $queryValidateEmployee = "SELECT id FROM employees WHERE id='$id' AND deleted_at IS NULL";
    
    $mqValidateEmployee = mysqli_query($db_conn, $queryValidateEmployee);
    
    $fetchValidateEmployee = mysqli_fetch_assoc($mqValidateEmployee, MYSQLI_ASSOC);
    
    if($tokens['status'] == 200 || $tokens['status'] == "200" || $tokens['status'] == 401 || $tokens['status'] == "401") {
        $tkn = $tokenizer->reCreate($token);
        $success = 1;
        $signupMsg = "Success";
    } else if (count($fetchValidateEmployee) < 1) {
        $status = $tokens['status'];
        $signupMsg = $tokens['msg'];
        $success = 0;
        $tkn="Unauthorized";
