...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    $q = mysqli_query($db_conn, "SELECT m.nama AS label, m.id, m.id AS value, m.hpp AS cogs, m.stock, 'pcs' AS metric_name FROM menu m JOIN categories c ON m.id_category=c.id WHERE m.id_partner='$partner_id' AND m.is_recipe=0 AND m.deleted_at IS NULL");
    ...
    if (mysqli_num_rows($q) > 0 || mysqli_num_rows($q1) > 0) {
        ...
        foreach ($res1 as $r) {
            ...
            $id = $r['metric_id'];
            $rawID = $r['value'];
            $qStock = mysqli_query($db_conn, "SELECT IFNULL(SUM(stock),0) AS stock FROM raw_material_stock WHERE deleted_at IS NULL AND id_raw_material='$rawID'");
            $stocks = mysqli_fetch_all($qStock, MYSQLI_ASSOC);
