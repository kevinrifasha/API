...
class EmployeeManager
{
  ...
  public function update(employee $emply)
  {
    ...
    $q = $this->_db->prepare("UPDATE `employees` SET `nik`='{$nik}',`nama`='{$nama}',`gender`='{$gender}',`phone`='{$phone}',`email`='{$email}',`pin`='{$pin}',`id_master`='{$id_master}',`id_partner`='{$id_partner}',`role_id`='{$role_id}',`pattern_id`='{$pattern_id}',`show_as_server`='{$show_as_server}' WHERE `id`='{$id}'");
    return $q->execute();
  }
  ...
  public function add(employee $emply)
  {
    ...
    $q = $this->_db->prepare("INSERT INTO `employees` SET `nik`='{$nik}',`nama`='{$nama}',`gender`='{$gender}',`phone`='{$phone}',`email`='{$email}',`pin`='{$pin}',`id_master`='{$id_master}',`id_partner`='{$id_partner}',`role_id`='{$role_id}',`pattern_id`='{$pattern_id}',`show_as_server`='{$show_as_server}'");
    $a = $q->execute();
    if ($a != false) {
  ...
  {
    $q = $this->_db->query("SELECT * FROM `employees` WHERE nik='{$nik}' AND deleted_at IS NULL");
  ...
  {
    $q = $this->_db->query("SELECT * FROM `employees` WHERE phone='{$phone}' AND deleted_at IS NULL");
  ...
  {
    $q = $this->_db->query("SELECT * FROM `employees` WHERE email='{$email}' AND deleted_at IS NULL");
  ...
  {
    $q = $this->_db->query("SELECT * FROM `employees` WHERE phone='{$phone}' AND pin='{$pin}' AND deleted_at IS NULL");
  ...
  {
    $q = $this->_db->query("SELECT * FROM `employees` WHERE email='{$email}' AND pin='{$pin}' AND deleted_at IS NULL");
