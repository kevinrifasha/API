...
foreach ($headers as $header => $value) {
    ...
    if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
        ...
        $q = mysqli_query($db_conn, "SELECT po.id, po.no, po.partner_id, po.supplier_id, po.total, p.name AS partnerName, s.name AS supplierName FROM purchase_orders po JOIN partner p ON p.id=po.partner_id JOIN suppliers s ON po.supplier_id=s.id WHERE po.partner_id='$partnerID' AND po.deleted_at IS NULL AND po.received=0 ORDER BY po.id DESC");
