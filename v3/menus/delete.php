...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    if(isset($obj['menuId']) && !empty($obj['menuId'])){

        $menu = $MenuManager->delete($obj['menuId']);
        if($menu==true){
            $menuVariantGroupsManager = new MenusVariantGroupsManager($db);

            //delete menu variant groups
            $menuVariantGroups  = $menuVariantGroupsManager->getByMenuId($obj['menuId']);
            if($menuVariantGroups!=false){
                foreach ($menuVariantGroups as $value) {
                    $today = date("Y-m-d H:i:s");
                    $value->setDeleted_at($today);
                    $update = $menuVariantGroupsManager->update($value);
                }
            }
            $success=1;
            $signupMsg="Success";
    ...
    $signupJson = json_encode(["success"=>$success,"msg"=>$signupMsg, "status"=>$status]);
