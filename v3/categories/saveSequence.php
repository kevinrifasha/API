...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    if(
        isset($obj->categories) && !empty($obj->categories)
            ...
            $obj->categories=json_decode($obj->categories);
            
            $categoryManager = new CategoryManager($db);
            $index = 1;
            foreach ($obj->categories as $value) {
                $category = new Category(array("id"=>$value->id,"id_master"=>$tokenDcrpt->masterID,"name"=>str_replace("'","''",$value->name),"sequence"=>$index));
                if($category!=false){
