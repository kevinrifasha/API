...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
  ...
  if($newDateFormat == 1){
    if($all !== "1") {
      $idMaster = null;
      $query = "SELECT SUM(qty) as value, nama as name, id FROM ( SELECT SUM(detail_transaksi.qty) AS qty, menu.nama, menu.id FROM detail_transaksi JOIN transaksi ON detail_transaksi.id_transaksi=transaksi.id JOIN menu ON menu.id=detail_transaksi.id_menu  WHERE transaksi.id_partner='$id' AND transaksi.deleted_at IS NULL AND transaksi.status<=2 and transaksi.status>=1 AND transaksi.paid_date BETWEEN '$dateFrom' AND '$dateTo' GROUP BY menu.id ";
    } else {
        $query = "SELECT SUM(qty) as value, nama as name, id FROM ( SELECT SUM(detail_transaksi.qty) AS qty, menu.nama, menu.id FROM detail_transaksi JOIN transaksi ON detail_transaksi.id_transaksi=transaksi.id JOIN menu ON menu.id=detail_transaksi.id_menu JOIN partner p ON p.id = transaksi.id_partner WHERE p.id_master = '$idMaster' AND transaksi.deleted_at IS NULL AND transaksi.status<=2 and transaksi.status>=1 AND transaksi.paid_date BETWEEN '$dateFrom' AND '$dateTo' GROUP BY menu.id ";
    }
    ...
    //         $query .= " UNION ALL ";
    //         $query .= " SELECT SUM(`$detail_transactions`.qty) AS qty, menu.nama, menu.id FROM `$detail_transactions` JOIN `$transactions` ON `$detail_transactions`.id_transaksi=`$transactions`.id JOIN menu ON menu.id=`$detail_transactions`.id_menu  WHERE `$transactions`.id_partner='$id' AND `$transactions`.deleted_at IS NULL AND `$transactions`.status<=2 and `$transactions`.status>=1 AND `$transactions`.paid_date BETWEEN '$dateFrom' AND '$dateTo' GROUP BY menu.id ";
    //     }
    // }
