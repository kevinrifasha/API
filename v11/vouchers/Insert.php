...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if(
        ...
        if (mysqli_num_rows($q) > 0) {
            ...
            $sqlInsert = "INSERT voucher SET code='{$code}', title='{$title}', description='{$description}', type_id='{$type_id}', is_percent='{$is_percent}', discount='{$discount}', enabled='{$enabled}', valid_from='{$valid_from}', valid_until='{$valid_until}', total_usage='{$total_usage}', img='{$img}', created_at= NOW(), prerequisite='{$prerequisite}'";
