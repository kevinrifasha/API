...
if (isset($tokenValidate['success']) && $tokenValidate['success'] == 0) {
    ...
    if($queryPartner && count($fetchPartner)){
        ...
        foreach($fetchPartner as $partner){
            ...
            if ($hppQ && mysqli_num_rows($hppQ) > 0) {
                ...
                $allGrossProfit += $arr['gross_income']; 
            }
            ...
            $allNetProfit += $arr['total_net_profit_after_ewallet_charge'];
            
            $data = ["id"=>$id, "name"=>$name,"gross_profit" => $arr['gross_income'], "hpp" => $arr['hpp'], "net_profit" => $arr['total_net_profit_after_ewallet_charge'] ?? 0];
            $array[$z]= $data;
            $z++;
        }
    ...
    $dataAll = ["id"=>"0", "name"=>"Semua Merchant","gross_profit" => $allGrossProfit, "hpp" => $allHpp, "net_profit" => $allNetProfit ?? 0];
