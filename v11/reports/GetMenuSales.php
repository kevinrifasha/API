...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    $sqlGetSales = mysqli_query($db_conn, "SELECT SUM(detail_transaksi.harga) AS sales, SUM(detail_transaksi.qty) AS qty, menu.nama AS nama FROM detail_transaksi JOIN transaksi ON detail_transaksi.id_transaksi=transaksi.id JOIN menu ON menu.id=detail_transaksi.id_menu ". $addQuery2 ." WHERE ". $addQuery1 ." AND transaksi.deleted_at IS NULL AND detail_transaksi.deleted_at IS NULL AND detail_transaksi.status!=4 AND transaksi.status IN(1,2) AND DATE(transaksi.paid_date) BETWEEN '$dateFrom' AND '$dateTo' GROUP BY menu.id ORDER BY sales DESC");
    ...
    if(mysqli_num_rows($sqlGetSales) > 0) {
        while($row2=mysqli_fetch_assoc($sqlGetSales)){
            ...
            if($most == false && $least==false){
                ...
                $total+= $qty2;
                $totalS+= $sales2;
            } else if($most == true && $least == false && $i < 10){
                array_push($arr, array("name" => "$namaMenu2", "qty" => $qty2, "sales"=>$sales2));
                $total+= $qty2;
                $totalS+= $sales2;
            } else if($least == true && $most == false && $i > $count - 10){
                array_push($arr, array("name" => "$namaMenu2", "qty" => $qty2, "sales"=>$sales2));
                $total+= $qty2;
                $totalS+= $sales2;
