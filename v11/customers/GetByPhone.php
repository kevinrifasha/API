...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    $transaksi = mysqli_query($db_conn, $queryTrans);
            ...
            $sqlGetUser =  "SELECT name, phone FROM `users` WHERE phone='$phone' AND deleted_at IS NULL";
        ...
        if (mysqli_num_rows($user) > 0) {
            ...
            $transactions1 = mysqli_fetch_all($sqlTransactionList, MYSQLI_ASSOC);
            ...
            $status = 200;
            $msg = "Nomor Telp. tidak terdaftar! "; 
            $success = 0; 
        }
    }else{ 
        $status = 400;
...
echo json_encode(["success"=>$success, "status"=>$status, "msg"=>$msg, "details"=>$details, "first"=>$first, "last"=>$last, "count"=>$count, "transactions"=>$transactions1, "point"=>$point]);