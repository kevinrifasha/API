...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    $id = $token->id_partner;
    $query = "";
    if(isset($data->isPickUpNotification)){
        $query = "UPDATE partner SET is_pickup_notification='$data->isPickUpNotification' WHERE id='$id'";
        $sql = mysqli_query($db_conn, $query);
     if($sql){
         $success = 1;
         $status = 200;
         if($data->isPickUpNotification=="1"||$data->isPickUpNotification==1){
