    ...
    if(isset($data->transactionsID) &&!empty($data->transactionsID) && isset($data->paymentMethod) &&!empty($data->paymentMethod) && isset($data->status) ){
        ...
        $data->paymentMethod!=11 || $data->paymentMethod!="11" || $data->paymentMethod!=12 || $data->paymentMethod!="12"){
            ...
            foreach ($transactionID as $value) {
                ...
                $insertOST = mysqli_query($db_conn, "INSERT INTO `order_status_trackings`(`transaction_id`, `status_before`, `status_after`, `created_at`) VALUES ('$value', '$tStatus', '$data->status', NOW())");
                ...
                if($update || $update1){
                    ...
                    if($data->status==2){
                        ...
                        foreach ($transactionID as $value) {
                            ...
                            $dateNow = date('d M Y');
                            $timeNow = date("h:i");
                        $query = "SELECT template FROM `email_template` WHERE nama='receipt_natta'";
