...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if(isset($_GET['from'])&&!empty($_GET['from']) && isset($_GET['to'])&&!empty($_GET['to'])){
        ...
        $q1 = mysqli_query($db_conn, "SELECT COUNT(t.id) as total_data  FROM transaksi t JOIN payment_method pm ON t.tipe_bayar = pm.id LEFT JOIN users u ON u.phone=t.phone JOIN partner p ON p.id = t.id_partner
        WHERE p.id='$token->id_partner' AND p.tenant_id=0 AND t.deleted_at IS NULL AND t.status>=1 AND tipe_bayar=5 
        OR p.id='$token->id_partner' AND t.deleted_at IS NULL AND t.status>=1 AND tipe_bayar=7 
        OR p.id='$token->id_partner' AND t.deleted_at IS NULL AND t.status>=1 AND tipe_bayar=8 
        OR p.id='$token->id_partner' AND t.deleted_at IS NULL AND t.status>=1 AND tipe_bayar=9
        ORDER BY jam DESC");
        
        if((int)$_GET['is_tenant']==1){
            if($_GET['is_centralized']==1){
                $param = " JOIN detail_transaksi ON detail_transaksi.id_transaksi=t.id JOIN menu ON menu.id=detail_transaksi.id_menu WHERE menu.id_partner='$token->id_partner' AND t.deleted_at IS NULL AND DATE(jam) BETWEEN '$from' AND '$to' GROUP BY t.id";
            }else{
                $param = " WHERE t.tenant_id='$token->id_partner' AND t.deleted_at IS NULL AND DATE(jam) BETWEEN '$from' AND '$to' ";
            }
        }else{
            $param = " WHERE p.id='$token->id_partner' AND t.deleted_at IS NULL AND DATE(jam) BETWEEN '$from' AND '$to' ";
        }
        $sql = "SELECT t.id, t.jam, t.phone, t.no_meja, t.status, t.total, t.id_voucher, t.id_voucher_redeemable, t.tipe_bayar, t.promo, t.diskon_spesial, t.employee_discount, t.point, t.queue, t.takeaway, t.notes, t.tax, t.service, t.charge_ur, pm.nama as payment_method, case when u.name is null then t.customer_name else u.name end AS uname, case when u.name is null then 1 else 0 end AS is_pos, t.program_discount, t.customer_email FROM transaksi t JOIN payment_method pm ON t.tipe_bayar = pm.id LEFT JOIN users u ON u.phone=t.phone JOIN partner p ON p.id = t.id_partner".$param." ORDER BY jam DESC ";
