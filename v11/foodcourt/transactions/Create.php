        ...
        && !empty($data->paymentMethod) ){
            ...
            if($boolQty==true){
                ...
                $sql = "START TRANSACTION;";
                    if($paymentMethod=='11' || $paymentMethod==11){
                        $sql .= "INSERT INTO transaksi(`id`, `jam`, `phone`, `id_partner`, `no_meja`, `status`, `total`, `tipe_bayar`, `promo`, `point`, `queue`, `takeaway`, `notes`, `id_foodcourt`, `tax`, `service`, `charge_ewallet`, `charge_xendit`, `charge_ur`, `id_voucher`, `id_voucher_redeemable`, `diskon_spesial`, `shift_id`, `program_discount`, `program_id`, `customer_name`, `customer_email`, `is_pos`) VALUES ('$id', '$today', '$phone', '$partnerID', '$tableCode', '1', '$total', '$paymentMethod', '$promo', '$point', '$is_queue', '$is_takeaway', '$notes', '$foodcourtID', '$tax', '$service', '$charge_ewallet', '$charge_xendit', '$charge_ur', '$id_voucher', '$id_voucher_redeemable', '$diskon_spesial', '$shiftID', '$program_discount', '$program_id', '$customer_name', '$customer_email', '$is_pos'); ";
                    }else{
                        ...
                        $sql .= "INSERT INTO transaksi(`id`, `jam`, `paid_date`, `phone`, `id_partner`, `no_meja`, `status`, `total`, `tipe_bayar`, `promo`, `point`, `queue`, `takeaway`, `notes`, `id_foodcourt`, `tax`, `service`, `charge_ewallet`, `charge_xendit`, `charge_ur`, `id_voucher`, `id_voucher_redeemable`, `diskon_spesial`, `shift_id`, `program_discount`, `program_id`, `customer_name`, `customer_email`, `is_pos`) VALUES ('$id', '$today', '$today', '$phone', '$partnerID', '$tableCode', '$status', '$total', '$paymentMethod', '$promo', '$point', '$is_queue', '$is_takeaway', '$notes', '$foodcourtID', '$tax', '$service', '$charge_ewallet', '$charge_xendit', '$charge_ur', '$id_voucher', '$id_voucher_redeemable', '$diskon_spesial', '$shiftID', '$program_discount', '$program_id', '$customer_name', '$customer_email', '$is_pos'); ";
                    }
                ...
                // if($insert){
                    ...
                    if (mysqli_multi_query($db_conn,$sql)) {
                        ...
                        // if($paymentMethod==5 || $paymentMethod=="5" || $paymentMethod==7 || $paymentMethod=="7" || $paymentMethod==8 || $paymentMethod=="8" || $paymentMethod==9 || $paymentMethod=="9" || $paymentMethod==11 || $paymentMethod=="11" ){
                            ...
                            $id_tenantsQ = mysqli_query($db_conn, "SELECT menu.id_partner FROM `detail_transaksi` JOIN menu ON detail_transaksi.id_menu=menu.id WHERE id_transaksi='$id' AND detail_transaksi.deleted_at IS NULL GROUP BY menu.id_partner");
                            if (mysqli_num_rows($id_tenantsQ) > 0) {
                                $id_tenants = mysqli_fetch_all($id_tenantsQ, MYSQLI_ASSOC);
                                foreach($id_tenants as $data_id_tenant){
                                    $id_tenant = $data_id_tenant['id_partner'];
                                    $devTokens = $db->getPartnerDeviceTokens($id_tenant);
                        ...
                        if(isset($data->qr_code) && !empty($data->qr_code)){
                            ...
                            $q = mysqli_query($db_conn,"UPDATE `temporary_qr` SET `status`='0' WHERE `code`='$qr_code'");
                        }
