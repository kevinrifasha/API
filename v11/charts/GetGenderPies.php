...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    if($all == "1") {
        $query = "SELECT COUNT(`transaksi`.id) AS counted, CASE WHEN users.Gender IS NOT NULL AND users.Gender!='' THEN users.Gender ELSE 'Belum isi' END gender FROM `transaksi` LEFT JOIN users ON users.phone=`transaksi`.phone JOIN partner p ON p.id = transaksi.id_partner WHERE DATE(`transaksi`.paid_date) BETWEEN '$dateFrom' AND '$dateTo' AND `transaksi`.deleted_at IS NULL AND p.id_master='$idMaster' AND transaksi.status IN(1,2) GROUP BY users.Gender ORDER BY users.Gender DESC";
    } else {
...
echo json_encode(["success"=>$success, "status"=>$status,"msg"=>$msg, "genderTransactionPercentage"=>$values]);
