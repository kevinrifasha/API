...
if (isset($tokens['success']) && $tokens['success'] == '0' || isset($tokens['success']) && $tokens['success'] == 0) {
    ...
    if ($all == "1") {
        $query = "SELECT SUM(detail_transaksi.qty) AS qty, menu.nama, menu.id AS menu_id FROM detail_transaksi JOIN transaksi ON detail_transaksi.id_transaksi = transaksi.id JOIN menu ON menu.id = detail_transaksi.id_menu JOIN partner p ON p.id = transaksi.id_partner WHERE p.id_master = '$idMaster' AND transaksi.deleted_at IS NULL AND transaksi.status IN(1, 2) AND DATE(transaksi.paid_date) BETWEEN '$dateFrom' AND '$dateTo' GROUP BY menu_id ORDER BY qty DESC LIMIT 5";
    } else {
...
echo json_encode(["success" => $success, "status" => $status, "msg" => $msg, "bestSeller" => $bestSeller]);
