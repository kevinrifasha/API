...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
  ...
  $json = file_get_contents('php://input');
    ...
    if(
        ...
        $q = mysqli_query($db_conn, "SELECT id FROM `membership_voucher` WHERE code='$code' AND deleted_at IS NULL");
        
        if (mysqli_num_rows($q) > 0) {
            $success =0;
            $status =204;
            $msg = "Kode voucher tidak boleh sama";
        } else {
            $sqlInsert = "INSERT membership_voucher SET code='{$code}', title='{$title}', description='{$description}', type_id='{$type_id}', is_percent='{$is_percent}', discount='{$discount}', enabled='{$enabled}', valid_from='{$valid_from}', valid_until='{$valid_until}', total_usage='{$total_usage}', img='{$img}', created_at= NOW(), prerequisite='{$prerequisite}', point='{$point}'";
