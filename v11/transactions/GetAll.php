...
if (isset($tokenValidate['success']) && $tokenValidate['success'] == 0) {
    ...
    if (isset($_GET['page']) && !empty($_GET['page']) && isset($_GET['load']) && !empty($_GET['load'])) {
        ...
        $q1 = mysqli_query($db_conn, "SELECT COUNT(t.id) as total_data  FROM transaksi t JOIN payment_method pm ON t.tipe_bayar = pm.id LEFT JOIN users u ON u.phone=t.phone WHERE t.id_partner='$token->id_partner' AND t.deleted_at IS NULL AND ((t.status <2 AND tipe_bayar NOT IN (1,10,2,3,4,6,11,14)) OR (t.status=1 AND tipe_bayar IN(1,10,2,3,4,6,11,14))) ORDER BY jam DESC");
        ...
        $q = mysqli_query($db_conn, "SELECT t.employee_discount_percent, t.is_pos as transaction_is_pos, t.id, t.jam, t.phone, t.no_meja, t.status, t.total, t.id_voucher, t.id_voucher_redeemable, t.tipe_bayar, t.promo, t.diskon_spesial, t.employee_discount, t.point, t.queue, t.takeaway, t.notes, t.tax, t.service, t.charge_ur, t.rounding, pm.nama as payment_method, case when u.name is null or t.is_helper = 1 or t.is_pos = 1 then t.customer_name else u.name end AS uname, case when u.name is null then 1 else 0 end AS is_pos, t.program_discount, t.customer_email, t.is_helper, t.surcharge_id FROM transaksi t JOIN payment_method pm ON t.tipe_bayar = pm.id LEFT JOIN users u ON u.phone=t.phone AND u.deleted_at IS NULL WHERE id_partner='$token->id_partner' AND t.deleted_at IS NULL AND ((t.status <2 AND tipe_bayar NOT IN (1,10,2,3,4,6,14)) OR (t.status=1 AND tipe_bayar IN(1,10,2,3,4,6,11, 14))) ORDER BY jam DESC LIMIT $start,$load");
        if (
            mysqli_num_rows($q) > 0
