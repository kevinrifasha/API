...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    $getRsvData = mysqli_query($db_conn, "SELECT 
      fr.id, 
      m.idmeja,
      fr.meja_id, 
      fr.table_group_id, 
    ...
    $rsv = mysqli_fetch_all($getRsvData, MYSQLI_ASSOC);
    $reservationStatus = $rsv[0]['reservationStatus'];
    $duration = $rsv[0]['duration'];
    $duration_metric = $rsv[0]['duration_metric'];
    $minimumTransaction = $rsv[0]['minimum_transaction'];
    $bookingPrice = $rsv[0]['booking_price'];
    $capacity = $rsv[0]['capacity'];
    $tableName = $rsv[0]['idmeja'];
    $tableID = $rsv[0]['meja_id'];
