...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if($category=="all"){
        ...
        $query = "SELECT  menu.id, menu.id_partner, menu.nama, menu.harga, menu.Deskripsi, menu.category, menu.id_category, menu.img_data, menu.enabled, menu.stock, menu.hpp, menu.harga_diskon, menu.is_variant, menu.is_recommended, menu.is_recipe, menu.is_auto_cogs, menu.thumbnail, menu.created_at, partner.name, categories.name as cname, CASE WHEN stock=0 THEN 0 ELSE 1 END AS tempFlag FROM menu JOIN partner ON menu.id_partner = partner.id JOIN categories ON categories.id = menu.id_category WHERE partner.id = '$id' AND menu.deleted_at IS NULL AND menu.enabled='1' AND menu.show_in_sf=1 GROUP BY menu.id ";
    ...
    if($category=="recomended"){
        ...
        $query = "SELECT  menu.id, menu.id_partner, menu.nama, menu.harga, menu.Deskripsi, menu.category, menu.id_category, menu.img_data, menu.enabled, menu.stock, menu.hpp, menu.harga_diskon, menu.is_variant, menu.is_recommended, menu.is_recipe, menu.is_auto_cogs, menu.thumbnail, menu.created_at, partner.name, categories.name as cname, CASE WHEN stock=0 THEN 0 ELSE 1 END AS tempFlag FROM menu JOIN partner ON menu.id_partner = partner.id JOIN categories ON categories.id = menu.id_category WHERE partner.id = '$id' AND menu.is_recommended=1  AND menu.deleted_at IS NULL AND menu.enabled='1' AND menu.show_in_sf=1 GROUP BY menu.id ";
...
echo json_encode(["success"=>$success, "status"=>$status, "msg"=>$msg, "menus"=>$arr, "q"=>$query]);
