...
while($row=mysqli_fetch_assoc($transaksi)){
    ...
    // if(($dateFromStr>=$table_name[1]) || ($dateToStr>=$table_name[2])){
        $query .= "UNION ALL " ;
        $query .=   "SELECT  `$transactions`.id, `$transactions`.jam, `$transactions`.paid_date, `$transactions`.phone, `$transactions`.customer_name, `$transactions`.reference_id, `$transactions`.id_partner, `$transactions`.shift_id, `$transactions`.no_meja, `$transactions`.no_meja_foodcourt, `$transactions`.status, `$transactions`.total, `$transactions`.id_voucher, `$transactions`.id_voucher_redeemable, `$transactions`.tipe_bayar, `$transactions`.promo, `$transactions`.diskon_spesial, `$transactions`.employee_discount, `$transactions`.point, `$transactions`.queue, `$transactions`.takeaway, `$transactions`.notes, `$transactions`.id_foodcourt, `$transactions`.tax, `$transactions`.service, `$transactions`.pre_order_id, `$transactions`.charge_ewallet, `$transactions`.charge_xendit, `$transactions`.charge_ur, `$transactions`.confirm_at, `$transactions`.status_callback, `$transactions`.callback_at, `$transactions`.callback_hit, `$transactions`.qr_string, `$transactions`.partner_note, `$transactions`.group_id, `$transactions`.rated, `$transactions`.created_at, partner.name, `$transactions`.program_discount, tenant.name AS tenant_name FROM `$transactions` JOIN partner ON `$transactions`.id_partner = partner.id LEFT JOIN partner tenant ON tenant.id=`$transactions`.tenant_id WHERE `$transactions`.phone='$token->phone' AND `$transactions`.deleted_at IS NULL AND `$transactions`.status <> 0 AND `$transactions`.status <> 1 AND `$transactions`.status <> 5 ";
    // }
}
