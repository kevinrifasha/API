...
class Token
{
  ...
  public function validate($tkn)
  {
    ...
    if (
      isset($tknDecrypt->phone) && !empty($tknDecrypt->phone)
    ) {
  ...
  public function reCreate($tkn)
  {
    ...
    if ($checker["status"] == 403) {
      ...
      $tkn = json_encode(['email' => $tknDecrypt->email, 'name' => $tknDecrypt->name, 'phone' => $tknDecrypt->phone, 'created_at' => $today, 'expired' => 500, "id" => $tknDecrypt->id, "email" => $tknDecrypt->email]);
    }
