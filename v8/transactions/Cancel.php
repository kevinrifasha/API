    ...
    if(
        ...
        foreach ($trasactionID as $value) {
            ...
            if (mysqli_num_rows($qT) > 0) {
                ...
                $selectPrevOST = mysqli_query($db_conn, "SELECT * FROM order_status_trackings WHERE transaction_id LIKE '%$id%' AND deleted_at IS NULL ORDER BY id DESC");
                ...
                $prevPaymentBefore = $fetchSelectPrevOST[0]["payment_method_before"];
                $prevPaymentAfter = $fetchSelectPrevOST[0]["payment_method_after"];

                $insertOST = mysqli_query($db_conn, "INSERT INTO `order_status_trackings`(`transaction_id`, `status_before`, `status_after`,`payment_method_before`,`payment_method_after`, `created_by`, `created_at`) VALUES ('$id', '$tStatus', '$obj->status', '$prevPaymentBefore', '$prevPaymentAfter','$token->id' , NOW())");
...
echo json_encode(["success"=>$success, "status"=>$status, "msg"=>$msg, "shift_id"=>$shift_id]);
?>