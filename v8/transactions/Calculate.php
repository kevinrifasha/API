    ...
    if( isset($data->partnerID)
        ...
        && !empty($data->detail)){
                ...
                if(isset($cart->status) && !empty($cart->status)){
                    if($cart->status==4){
                        ...
                        if(isset($cart->is_program) && !empty(isset($cart->is_program)) && $cart->is_program!=0){
                            ...
                            $total += (int) $cart->price *(int) $cart->qty;
                        }
                    }
                }else{
                    if(isset($cart->is_program) && !empty(isset($cart->is_program)) && $cart->is_program!=0){
                            ...
                            if(isset($cart->status) && !empty($cart->status)){
                                if($cart->status==4){
                                    ...
                                    if(isset($cart->is_program) && !empty($cart->is_program)){
                                        ...
                                        if($cart->id==$prerequisite->menu_id){
                                            $tempTot += (int) $cart->price *(int) $cart->qty;
                                        }
                                    }
                                }
            ...
            if(
                ...
                (isset($data->is_program_discount) && !empty($data->is_program_discount) && ($data->is_program_discount != "0"))
                ){

                $resDp = $programsB->ProgramDiscount($data->partnerID, $transaction_type, $total, $dataDetail);

                $program_id = $resDp['id'];
                $discountType = $resDp['discountType'];
                $program_discount = (int)$resDp['discount'];
                $program_title = $resDp['title'];
                $program_payment_method = $resDp['payment_method'];
                $percent_program = $resDp['percentage'];
                $menus_program = $resDp['menuList'];
            }
            ...
            foreach ($dataDetail as $cart) {
                ...
                if(isset($cart->status) && !empty($cart->status)){
                    if($cart->status==4){
                        ...
                        $menuQ = mysqli_query($db_conn, "SELECT nama, harga, stock, is_recipe FROM `menu` WHERE id='$cart->id' AND enabled='1'");
            ...
            $iDD = 0;
            foreach ($dataDetail as $cart) {
                $dataDetail[$iDD]->harga = $cart->qty*$cart->price;
                $iDD +=1;
            }
            ...
            $program_payment_method = json_decode($program_payment_method);
            
            $details = [];
            if(count($menus_program) > 0) {
                 foreach ($dataDetail as $cart) {
                     foreach ($menus_program as $value) {
                         if ($cart->id == $value['id_menu']) {
                             $cart->is_program = $program_id;
                            //  $cart->harga -= $value['discountTotalMenu'];
                            //  $cart->totalPrice -= $value['discountTotalMenu'];
                            //  $cart->price -= $value['discountPerQty'];
                         }
                     }
                     
                     array_push($details, $cart);
                 }
            }
            ...
            if(isset($data->rounding_down_below) && (isset($data->is_rounding) &&( $data->is_rounding == "1" ||$data->is_rounding == 1))){
                if($data->rounding_down_below !== 0 || $data->rounding_down_below !== "0" ){
                    $roundingData = (int) $data->rounding_down_below; 
                    $roundingString = (string) $data->rounding_down_below;
                    if(strlen($roundingString) == 3){
                        if($gtotal % 1000 <= $roundingData){
                            $rounding = $gtotal - ($gtotal % 1000);
                        } else {
                            $rounding = $gtotal + (1000 -  ($gtotal % 1000));
                        }
                    } else if (strlen($roundingString) == 2){
                        if($gtotal % 100 <= $roundingData){
                            $rounding = $gtotal - ($gtotal % 100);
                        } else {
                            $rounding = $gtotal + (100 -  ($gtotal % 100));
                        }
                    } else if (strlen($roundingString) == 1){
                        if($gtotal % 10 <= $roundingData){
                            $rounding = $gtotal - ($gtotal % 10);
                        } else {
                            $rounding = $gtotal + (10 -  ($gtotal % 10));
                        }
                    } 
                } else {
                    $rounding = $gtotal;
