...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if(
        ...
        $sql = mysqli_query($db_conn, "SELECT tipe_bayar, status FROM transaksi WHERE id = '$transactionID' AND deleted_at IS NULL");
        $getTransc = mysqli_fetch_all($sql, MYSQLI_ASSOC);
        $transcStatus = $getTransc[0]['status'];
        $tipe_bayar = $getTransc[0]['tipe_bayar'];
        ...
        if($transcStatus == 0) {
            ...
            foreach ($trasactionID as $value) {
                if(!empty($value)){
                    ...
                    $allTrans = mysqli_query($db_conn, "SELECT t.id, t.id_partner, t.queue, t.status, t.jam, t.phone, t.no_meja, t.total, t.id_voucher, t.id_voucher_redeemable, t.tipe_bayar, t.promo, t.program_discount, t.diskon_spesial, t.point, t.takeaway, t.notes, t.tax, t.service, t.charge_ur, t.rounding, p.nama AS payment_method, u.name AS uname, t.diskon_spesial, invoices.id AS invoice_id, invoices.invoice_code, t.tenant_id, invoice_details.id AS invoice_details_id FROM transaksi t JOIN payment_method p ON p.id=t.tipe_bayar JOIN users u ON t.phone=u.phone LEFT JOIN invoice_details ON invoice_details.transaction_id=t.id LEFT JOIN invoices ON invoices.id=invoice_details.invoice_id WHERE t.id='$id'");
                    $invoice_id = "0";
                    $invoice_code = "";
                    $tenant_id = "";
                    ...
                    if( $obj->paymentMethod=='1' || $obj->paymentMethod==1 || $obj->paymentMethod=='2' || $obj->paymentMethod==2 || $obj->paymentMethod=='3' || $obj->paymentMethod==3 || $obj->paymentMethod=='4' || $obj->paymentMethod==4 || $obj->paymentMethod=='5' || $obj->paymentMethod==5 || $obj->paymentMethod=='6' || $obj->paymentMethod==6 || $obj->paymentMethod=='7' || $obj->paymentMethod==7 || $obj->paymentMethod=='8' || $obj->paymentMethod==8 || $obj->paymentMethod=='9' || $obj->paymentMethod==9 || $obj->paymentMethod=='10' || $obj->paymentMethod==10 || $obj->paymentMethod=='14' || $obj->paymentMethod==14){
                        $shiftID =(int)getShiftID($partnerID, $db_conn);
                    }
                    ...
                    $qDetails = mysqli_query($db_conn, "SELECT dt.qty AS quantity, dt.harga_satuan AS price, dt.id_menu AS id, m.nama AS name FROM detail_transaksi dt JOIN menu m ON dt.id_menu=m.id WHERE dt.id_transaksi='$id' AND dt.deleted_at IS NULL");
                    ...
                    $il=0;
                        ...
                        if($shiftID!="0"){
                            $sql .= "UPDATE transaksi SET tipe_bayar='$obj->paymentMethod', id='$newID', status='$status', shift_id='$shiftID', updated_at=NOW() WHERE id='$obj->transactionID' ; ";
                        }else{
                        ...
                        if($paymentMethod==1 || $paymentMethod=="1" || $paymentMethod==2 || $paymentMethod=="2" || $paymentMethod==3 || $paymentMethod=="3" || $paymentMethod==4 || $paymentMethod=="4" || $paymentMethod==10 || $paymentMethod=="10" || $obj->paymentMethod=='14' || $obj->paymentMethod==14){
                            ...
                            $tservice = round(($total-$promo-$diskon_spesial)*$service/100);
                            $ttax = round(($total-$promo-$diskon_spesial+$tservice+$charge_ur)*$tax/100);
    
                            $amount = (int) round($total-$promo-$diskon_spesial+$tservice+$ttax+$delivery_fee+$charge_ur-$program_discount+$rounding);
                            $gamount+=$amount;
                            $phone1 = substr($phone, 1);
                            $phone1 = '+62'.$phone1;
                            $redirect_url = $_ENV["XENDIT_REDIRECT_URL"] . "success?transactionID=" . $newID . "&status=" . "1";
                            if(empty($tenant_id)){
                                if($paymentMethod=="1" || $paymentMethod==1 || $paymentMethod=="3" || $paymentMethod==3 ||$paymentMethod=="4" || $paymentMethod==4 ||$paymentMethod=="10" || $paymentMethod==10 || $paymentMethod=="14" || $paymentMethod=="14"){
                                if($paymentMethod=="1" || $paymentMethod==1){
                                    ...
                                    $params = [
                                        ...
                                        if (
                                            ...
                                            $updateQR = mysqli_query($db_conn, "UPDATE transaksi SET qr_string='$qrString' WHERE id='$newID'");
                                        }
        
                                        $UpdateCallback = mysqli_query(
                                            $db_conn,
                                            "INSERT INTO `xendit_callbacks`(`transaction_id`, `value`, `created_at`) VALUES ('$newID', '$result', NOW())"
                                        );
                                        curl_close($ch);
    ...
    // 
            if(!empty($tenant_id) && !empty($phone)){
                ...
                if($paymentMethod=="1" || $paymentMethod==1){
                    $params = [
                     ...
                     $redirect_url = $_ENV["XENDIT_REDIRECT_URL"] . "success?transactionID=" . $new_invoice_code . "&status=" . "1";
                    $params = [
                        'external_id' => $new_invoice_code,
                        'reference_id' => $new_invoice_code,
                        'currency' => 'IDR',
            ...
            $explodeNewID = explode("-",$newID);
            $basicTrxId = $explodeNewID[0];
            
            $selectPrevOST = mysqli_query($db_conn, "SELECT * FROM order_status_trackings WHERE transaction_id LIKE '%$basicTrxId%' AND deleted_at IS NULL ORDER BY id DESC");
            
            $fetchSelectPrevOST = mysqli_fetch_all($selectPrevOST, MYSQLI_ASSOC);
            
            $prevStatBefore = $fetchSelectPrevOST[0]["status_before"];
            $prevStatAfter = $fetchSelectPrevOST[0]["status_after"];
            
            $insertOST = mysqli_query($db_conn, "INSERT INTO `order_status_trackings`(`transaction_id`, `status_before`, `status_after`, `payment_method_before` ,`payment_method_after`, `created_by` ) VALUES ('$newID', '$prevStatBefore','$prevStatAfter' ,'$tipe_bayar', '$paymentMethod', '$token->id')");
        } else {
            $success = 0;
            $msg = "Gagal ubah metode bayar";
            $status = 400;
        }
