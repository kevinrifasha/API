...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if( isset($obj->data) ){
        ...
        foreach ($obj->data as $data) {
            ...
            $service = getService($tenant_id, $db_conn);
            $tax = getTaxEnabled($tenant_id, $db_conn);
            ...
            $charge_ur =(int)getChargeUr($status, $hide, $db_conn, $tenant_id);
            if($is_delivery=='1' || $is_delivery==1){
                if (strpos($delivery_detail, 'Kurir Pribadi') !== false) {
                }else{
                    $charge_ur =(int)getChargeUrShipper($status, $hide, $db_conn, $tenant_id);
                }
            }
            if($paymentMethod=='11' || $paymentMethod==11 ){
                $status = 5;
                $shiftID =(int)getShiftID($tenant_id, $db_conn);
            }
            ...
            if($boolQty==true){
                ...
                if($paymentMethod=='11' || $paymentMethod==11 || $paymentMethod=='1' || $paymentMethod==1 || $paymentMethod=='2' || $paymentMethod==2 || $paymentMethod=='3' || $paymentMethod==3 || $paymentMethod=='4' || $paymentMethod==4 || $paymentMethod=='6' || $paymentMethod==6 || $paymentMethod=='10' || $paymentMethod==10){
                    ...
                    $sql .= "INSERT INTO transaksi(`id`, `jam`, `phone`, `id_partner`, `no_meja`, `status`, `total`, `tipe_bayar`, `promo`, `point`, `queue`, `takeaway`, `notes`, `id_foodcourt`, `tax`, `service`, `charge_ewallet`, `charge_xendit`, `charge_ur`, `id_voucher`, `id_voucher_redeemable`, `diskon_spesial`, `shift_id`, `program_discount`, `program_id`, `tenant_id`) VALUES ('$id', '$today', '$phone', '$partnerID', '$tableCode', '$status', '$total', '$paymentMethod', '$promo', '$point', '$is_queue', '$is_takeaway', '$notes', '$foodcourtID', '$tax', '$service', '$charge_ewallet', '$charge_xendit', '$charge_ur', '$id_voucher', '$id_voucher_redeemable', '$diskon_spesial', '$shiftID', '$program_discount', '$program_id', '$tenant_id'); ";
                }else{
                    $sql .= "INSERT INTO transaksi(`id`, `jam`, `phone`, `id_partner`, `no_meja`, `status`, `total`, `tipe_bayar`, `promo`, `point`, `queue`, `takeaway`, `notes`, `id_foodcourt`, `tax`, `service`, `charge_ewallet`, `charge_xendit`, `charge_ur`, `id_voucher`, `id_voucher_redeemable`, `diskon_spesial`, `program_discount`, `program_id`, `tenant_id`) VALUES ('$id', '$today', '$phone', '$partnerID', '$tableCode', '$status', '$total', '$paymentMethod', '$promo', '$point', '$is_queue', '$is_takeaway', '$notes', '$foodcourtID', '$tax', '$service', '$charge_ewallet', '$charge_xendit', '$charge_ur', '$id_voucher', '$id_voucher_redeemable', '$diskon_spesial', '$program_discount', '$program_id', '$tenant_id'); ";
                }
...
$delivery_id="0";
                ...
                // if($insert){
                    addPoint($phone, $total, $tenant_id ,$db_conn);
                    ...
                    if (mysqli_multi_query($db_conn,$sql)) {
                        ...
                        if($paymentMethod==5 || $paymentMethod=="5" || $paymentMethod==7 || $paymentMethod=="7" || $paymentMethod==8 || $paymentMethod=="8" || $paymentMethod==9 || $paymentMethod=="9" || $paymentMethod==11 || $paymentMethod=="11" ){
                            ...
                            foreach ($devTokens as $val) {
                                $mID = $db->getMembership($tenant_id, $phone);
                                if($mID==0){
                                    $isMembership = false;
                                }else{
                                    $isMembership = true;
