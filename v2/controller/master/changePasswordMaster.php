    ...
    if(isset($_POST['password']))
    {
       ...
       isset($_POST['confpass']))
        ...
        if(strlen($password) <= 32 && strlen($password) >= 8)
        {
                ...
                {   
                    $oldPass = mysqli_query($db_conn, "SELECT `created_at` FROM `reset_password` WHERE md5(token) = '$phone'");
                    ...
                    if($ca == $today){
                        $temp = $db->updatePasswordMaster($password, $phone);
                        if($temp == USER_UPDATED){
                            echo "Update Password Successfully";
                            echo "<script>if(confirm('Your Password Sucessfully Updated. Now Login')){document.location.href='http://master.ur-hub.com'};</script>";
                            // header('Location: http://master.ur-hub.com');
                        }else{
