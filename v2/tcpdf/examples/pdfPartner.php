...
// $dateFirstDb = date('2020-09-01');
// $dateLastDb = date('2020-09-t');
...
$transaksi = mysqli_query($db_conn, "SELECT transaksi.* FROM transaksi  WHERE MD5(id_partner)='$id' AND transaksi.status<=2 and transaksi.status>=1 AND DATE(jam) BETWEEN '$dateFirstDb' AND '$dateLastDb'");
...
//   $subtotalAll =(($ovo+$gopay+$dana+$linkaja+$sakuku)-(ceil(($ovo+$gopay+$dana+$linkaja+$sakuku)*$charge_ewallet/100)+(ceil((($ovo+$gopay+$dana+$linkaja+$sakuku)*$charge_ewallet/100)*$taxtype/100)))+$tunaiDebit+$creditCard);
  ...
  $html .= '<b>Biaya atas Pendapatan<b><br>'; 
	...
	$html .='<tr><td>   Payment  :</td><td style="text-align:right;border-bottom-width:thin;">'.rupiah($sumPoint).'</td></tr>';
  ...
  $getHarga = mysqli_query($db_conn, "SELECT variant.* FROM `variant` JOIN variant_group ON variant.id_variant_group=variant_group.id JOIN menu ON menu.id = variant_group.id_menu WHERE MD5(menu.id_partner)='$id'");
