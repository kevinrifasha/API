...
$menu = mysqli_query($db_conn, "SELECT menu.nama,menu.harga,menu.hpp,SUM(transaksi_detail_tenant.qty*transaksi_detail_tenant.harga_satuan) AS total_harga,SUM(transaksi_detail_tenant.qty) as qtyTotal FROM menu join transaksi_detail_tenant on menu.id=transaksi_detail_tenant.id_menu JOIN transaksi_tenant ON transaksi_detail_tenant.id_transaksi_tenant=transaksi_tenant.id join transaksi_foodcourt on transaksi_tenant.id_transaksi_fc=transaksi_foodcourt.id WHERE MD5(transaksi_foodcourt.id_foodcourt)='$id' AND transaksi_foodcourt.status<=2 AND transaksi_foodcourt.status>=1 AND DATE(transaksi_foodcourt.created_at) BETWEEN '$dateFirstDb' AND '$dateLastDb' GROUP BY menu.nama order by SUM(transaksi_detail_tenant.qty) DESC");
$transaksi = mysqli_query($db_conn, "SELECT transaksi_foodcourt.* FROM transaksi_foodcourt WHERE MD5(transaksi_foodcourt.id_foodcourt)='$id' AND transaksi_foodcourt.status <= 2 transaksi_foodcourt.status>=1 AND DATE(created_at) BETWEEN '$dateFirstDb' AND '$dateLastDb'");
...
//   $subtotalAll =(($ovo+$gopay+$dana+$linkaja+$sakuku)-(ceil(($ovo+$gopay+$dana+$linkaja+$sakuku)*$charge_ewallet/100)+(ceil((($ovo+$gopay+$dana+$linkaja+$sakuku)*$charge_ewallet/100)*$taxtype/100)))+$tunaiDebit+$creditCard);
  ...
  $html .='<tr><td style="text-align:right">Total</td><td style="text-align:right">'.rupiah($sumTotal).'</td></tr>';
  $html .="</table><br><br>";
  ...
  $html .= '<b>Biaya atas Pendapatan<b><br>'; 
	...
	$html .='<tr><td style="text-align:right">Total</td><td style="text-align:right">'.rupiah($promo).'</td></tr>';
	$html .="</table><br><br>";
	$html .= '
	<table border="1" cellspacing="0" cellpadding="3" border ="0">
	...
	$html .='<tr><td style="text-align:right">Total</td><td style="text-align:right">'.rupiah($sumTax+$sumService-(ceil(($ovo+$gopay+$dana+$linkaja+$sakuku)*$charge_ewallet/100)+(ceil((($ovo+$gopay+$dana+$linkaja+$sakuku)*$charge_ewallet/100)*$taxtype/100))+$sumCharge_ur)).'</td></tr>';  
	$html .='<tr><td></td></tr>';
  ...
  while($rowmenu=mysqli_fetch_assoc($menu)){
	...
	$html .='<tr nobr="true"><td width="5%">'.$n.'</td><td width="23%">'.$rowmenu['nama'].'</td><td width="17%">'.rupiah($rowmenu['hpp']).'</td><td width="17%">'.rupiah($rowmenu['harga']).'</td><td width="8%">'.$rowmenu['qtyTotal'].'</td><td width="19%">'.rupiah($rowmenu['total_harga']).' </td></tr>';
	$n++;
  }
  $html .="</table>";
