...
$transaksi = mysqli_query($db_conn, "SELECT transaksi.* FROM transaksi  WHERE MD5(id_partner)='$id' AND status <= 2 AND status >= 1 AND DATE(jam)='$dateNowDb' ");
...
while($row=mysqli_fetch_assoc($transaksi)){
  ...
  $html .= '<b>Biaya atas Pendapatan<b><br>'; 
	...
	$html .='<tr><td>   Points Payments :</td><td style="text-align:right;border-bottom-width:thin;">'.rupiah($sumPoint).'</td></tr>';
