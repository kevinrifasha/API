...
header("Content-Disposition: attachment; filename=Laporan_Transaksi_\"$dateFormat\".xls");
   require '../db_connection.php';
	...
	<table border="1">
		...
		// koneksi database
		$trans = mysqli_query($db_conn, "SELECT transaksi.*,foodcourt.name as namaPartner FROM transaksi_foodcourt join foodcourt ON transaksi_foodcourt.id_foodcourt=foodcourt.id WHERE MD5(foodcourt.id)='$id' AND transaksi_foodcourt.status<= 2 AND transaksi_foodcourt.status>=1 AND DATE(created_at)='$dateNowDb' AND TIME(created_at) BETWEEN '$jamBuka' AND '$jamTutup'");
    $no = 1;
    $total=0;
    $promo=0;
    $ovo=0;
...
<br><br>
  ...
  <?php 
    ...
    $sumewallet=0;
    $sumecash=0;
    $tipe_bayar = mysqli_query($db_conn, "SELECT transaksi.*,foodcourt.name as namaPartner FROM transaksi_foodcourt join foodcourt ON transaksi_foodcourt.id_foodcourt=foodcourt.id WHERE MD5(foodcourt.id)='$id' AND transaksi_foodcourt.status<= 2 AND transaksi_foodcourt.status>=1 AND DATE(created_at)='$dateNowDb' AND TIME(created_at) BETWEEN '$jamBuka' AND '$jamTutup'");
