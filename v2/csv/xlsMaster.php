...
header("Content-Disposition: attachment; filename=Laporan_Transaksi_\"$dateFormat\".xls");
   require '../db_connection.php';
    ...
    $partner = mysqli_query($db_conn, "SELECT name FROM master WHERE MD5(id)='$id';");
  ...
  <b>Periode: <?php echo $first_day_last_month;?> sampai dengan <?php echo $last_day_last_month;?><b>
	<table border="1">
		...
		// koneksi database
		$trans = mysqli_query($db_conn, "SELECT transaksi.*,partner.name as namaPartner FROM transaksi join partner ON transaksi.id_partner=partner.id WHERE MD5(partner.id_master)='$id' AND transaksi.status<=2 and transaksi.status>=1 AND DATE(jam) BETWEEN '$dateFirstDb' AND '$dateLastDb'");
    $no = 1;
    $total=0;
    $promo=0;
    $ovo=0;
