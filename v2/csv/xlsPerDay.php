...
header("Content-Disposition: attachment; filename=Laporan_Transaksi_\"$dateFormat\".xls");
   require '../db_connection.php';
    ...
    //  $dateLastDb = date('Y-m-t');
     $partner = mysqli_query($db_conn, "SELECT name,jam_buka,jam_tutup FROM partner WHERE MD5(id)='$id'");
  ...
  <b>Tanggal: <?php echo $dateNow;?><b>
	<table border="1">
		...
		// koneksi database
		$trans = mysqli_query($db_conn, "SELECT transaksi.*,partner.name as namaPartner FROM transaksi join partner ON transaksi.id_partner=partner.id WHERE MD5(partner.id)='$id' AND transaksi.status<=2 and transaksi.status>=1 AND DATE(jam)='$dateNowDb' AND TIME(jam) BETWEEN '$jamBuka' AND '$jamTutup'");
    $no = 1;
    $total=0;
    $promo=0;
    $ovo=0;
