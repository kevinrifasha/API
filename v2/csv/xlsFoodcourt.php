...
header("Content-Disposition: attachment; filename=Laporan_Transaksi_\"$dateFormat\".xls");
   require '../db_connection.php';
    ...
    $partner = mysqli_query($db_conn, "SELECT name FROM foodcourt WHERE MD5(id)='$id';");
  ...
  <b>Periode: <?php echo $first_day_last_month;?> sampai dengan <?php echo $last_day_last_month;?><b>
	<table border="1">
		...
		// koneksi database
		$trans = mysqli_query($db_conn, "SELECT transaksi_foodcourt.*,foodcourt.name as namaPartner FROM transaksi_foodcourt join foodcourt ON transaksi_foodcourt.id_foodcourt=foodcourt.id WHERE MD5(foodcourt.id)='$id' AND transaksi_foodcourt.status <= 2 AND transaksi_foodcourt.status >= 1 AND DATE(transaksi_foodcourt.created_at) BETWEEN '$dateFirstDb' AND '$dateLastDb'");
    $no = 1;
    $total=0;
    $promo=0;
    $ovo=0;
...
<br><br>
  ...
  <?php 
    ...
    $sumewallet=0;
    $sumecash=0;
    $tipe_bayar = mysqli_query($db_conn, "SELECT transaksi_foodcourt.*,foodcourt.name as namaPartner FROM transaksi_foodcourt join foodcourt ON transaksi_foodcourt.id_foodcourt=foodcourt.id WHERE MD5(foodcourt.id)='$id' AND transaksi_foodcourt.status <= 2 AND transaksi_foodcourt.status >=1 AND DATE(transaksi_foodcourt.created_at) BETWEEN '$dateFirstDb' AND '$dateLastDb'");
    while ($rowtypeBayar = mysqli_fetch_assoc($tipe_bayar)) {
      ...
      if($rowtypeBayar['tipe_bayar'] == 1 || $rowtypeBayar['tipe_bayar'] == '1') {
        ...
        $creditCard += $withTax - $rowtypeBayar['charge_ur'];
        $sumecash+=$withTax;
      }else if($rowtypeBayar['tipe_bayar'] == 8 || $rowtypeBayar['tipe_bayar'] == '8'){
        $debitCard += $withTax - $rowtypeBayar['charge_ur'];
        $sumecash+=$withTax;
      }
  ...
  <tr>
      ...
      <td class="align-right" width="20%" class="purchase_item"><span class="f-fallback"><?php echo rupiah(($ovo+$gopay+$dana+$linkaja+$sakuku)-(ceil(($ovo+$gopay+$dana+$linkaja+$sakuku)*$charge_ewallet/100)+(ceil((($ovo+$gopay+$dana+$linkaja+$sakuku)*$charge_ewallet/100)*$taxtype/100)))+$tunaiDebit+$creditCard) ?></span></td>
    </tr>
    <tr>
        <td width="80%" class="purchase_item"><span class="f-fallback"></span></td>
        <td class="align-right" width="20%" class="purchase_item"><span class="f-fallback"></span></td>
