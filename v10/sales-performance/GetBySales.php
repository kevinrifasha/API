...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    $salesInfo = [];
    $i=1;
    $sqlSales = "SELECT u.name, u.email, u.phone FROM sa_users u WHERE u.id='$id'";
    ...
    $sqlByDate = mysqli_query($db_conn, "SELECT COUNT(v.id) AS counter,DATE(v.created_at) AS date FROM sa_visitations v WHERE v.deleted_at IS NULL ".$extraQuery." GROUP BY DATE(v.created_at) ORDER BY DATE(v.created_at) ASC");
    ...
    $salesInfo = mysqli_fetch_all($sqlSales, MYSQLI_ASSOC);
    ...
    if(mysqli_num_rows($sql) > 0) {
        ...
        foreach($wp as $x){
            $resWP[$i]=$x;
            $totalWPTarget +=(int)$x['target'];
            $totalWPCount +=(int)$x['visited'];
            $i++;
        }
        $success = 1;
        $status = 200;
...
echo json_encode(["success"=>$success, "status"=>$status,"msg"=>$msg, "data"=>$data, "total"=>$totalVisits, "package"=>$resPackage, "totalPackage"=>$totalPackage, "pos"=>$resPOS, "totalPOS"=>$totalPOS, "visitByDate"=>$visitByDate, "workPlans"=>$wp, "totalWPTarget"=>$totalWPTarget, "totalWPCount"=>$totalWPCount, "totalMerchant"=>$totalMerchant,"salesInfo"=>$salesInfo, "id"=>$id, "visits"=>$data]);
?>
