...
if((isset($tokenDecoded->masterID) && !empty($tokenDecoded->masterID)) || (isset($tokenDecoded->id_master) && !empty($tokenDecoded->id_master)) || (isset($tokenDecoded->phone) && !empty($tokenDecoded->phone))){
    ...
    if((isset($_GET['oid']) && !empty($_GET['oid']))
        || (isset($_GET['eid']) && !empty($_GET['eid']))
    ){
        if(isset($_GET['oid']) && !empty($_GET['oid'])){
            curl_setopt($ch, CURLOPT_URL, 'https://'.$_ENV['SHIPPER_URL'].'/public/v1/awbs/generate?apiKey='.$_ENV['SHIPPER_API_KEY'].'&oid='.$_GET['oid']);
        }else{
            curl_setopt($ch, CURLOPT_URL, 'https://'.$_ENV['SHIPPER_URL'].'/public/v1/awbs/generate?apiKey='.$_ENV['SHIPPER_API_KEY'].'&eid='.$_GET['eid']);
        }
