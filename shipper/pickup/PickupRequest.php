...
if(
    isset($obj['datePickup']) && !empty($obj['datePickup'])
    && isset($obj['orderIds']) && !empty($obj['orderIds'])
    && isset($obj['agentId']) && !empty($obj['agentId'])
    ){
        $val = "datePickup=".$obj['datePickup'];
        foreach ($obj['orderIds'] as $value) {
            $val .= "&orderIds[]=".$value;
        }
        $val .= "&agentId=".$obj['agentId'];
        var_dump($val);
        $url = 'https://'.$_ENV['SHIPPER_URL'].'/public/v1/pickup?apiKey='.$_ENV['SHIPPER_API_KEY'];
