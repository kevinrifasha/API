...
$ch = curl_init();
if(
    isset($_GET['origin']) && !empty($_GET['origin'])
    && isset($_GET['destination']) && !empty($_GET['destination'])
    && isset($_GET['length']) && !empty($_GET['length'])
    && isset($_GET['width']) && !empty($_GET['width'])
    && isset($_GET['height']) && !empty($_GET['height'])
    && isset($_GET['weight']) && !empty($_GET['weight'])
    && isset($_GET['price']) && !empty($_GET['price'])
    ){
        $url = 'https://'.$_ENV['SHIPPER_URL'].'/public/v1/domesticRates?apiKey='.$_ENV['SHIPPER_API_KEY'].'&o='.$_GET['origin'].'&d='.$_GET['destination'].'&l='.$_GET['length'].'&w='.$_GET['width'].'&h='.$_GET['height'].'&wt='.$_GET['weight'].'&v='.$_GET['price'];
        ...
        if(isset($_GET['origincoord']) && !empty($_GET['origincoord'])){
            $url .= '&originCoord='.$_GET['origincoord'];
        }
        if(isset($_GET['destinationcoord']) && !empty($_GET['destinationcoord'])){
            $url .= '&destinationCoord='.$_GET['destinationcoord'];
        }
    ...
    if($arr['status']==="success"){
        ...
        echo json_encode(["status"=>200, "success"=>1, "data"=>$arr['data']->rates]);
    }else{
