...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if( isset($obj->data) ){
        ...
        foreach($obj->data as $data){
            ...
            $pservice =  getService($data->partnerID, $db_conn);
            $service = ceil(($total+$totalProgram-$promo-$program_discount-$diskon_spesial)*$pservice/100);
            $ptax =  getTaxEnabled($data->partnerID, $db_conn);
            ...
            $gtotal = $total+$totalProgram-$promo-$diskon_spesial-$program_discount+$charge_ur+$service+$tax;
            ...
            // if($totalProgram==0){
                ...
                CASE
                    ...
                    ELSE 1
                END AS active
                FROM `programs` WHERE `partner_id`='$data->partnerID' AND master_program_id='1' AND `deleted_at` IS NULL AND `enabled`='1' AND '$today' BETWEEN `valid_from` AND  `valid_until` AND ((`start_hour`='00:00:00' AND `end_hour`='00:00:00') OR '$time' BETWEEN `start_hour` AND `end_hour`) AND (`day`='' OR `day` IS NULL OR `day` LIKE '%$dayNameInd%') ORDER BY active DESC, need_extra DESC");
