...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    $q = mysqli_query($db_conn, "SELECT `id`, `code`, `title`, `description`, `type_id`, `is_percent`, `discount`, `category`, `enabled`, `valid_from`, `valid_until`, `total_usage`, `prerequisite`, `master_id`, `partner_id` AS partnerID, partner_id, `img` from voucher WHERE valid_until>=NOW() AND enabled=1 AND deleted_at IS NULL AND show_in_sf=1");
