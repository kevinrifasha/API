...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if(isset($data->redeemByCode) && !empty($data->redeemByCode)){
        ...
        foreach($id_voucher_redeemables as $id_voucher_redeemable){
            $q = mysqli_query($db_conn, "SELECT master_id AS id_master, prerequisite FROM `redeemable_voucher` WHERE code='$id_voucher_redeemable' AND enabled='1' AND deleted_at IS NULL");
            if (mysqli_num_rows($q) > 0) {
                $voucher = mysqli_fetch_assoc($q);
                $maxPerUser = json_decode($voucher['prerequisite'])->order;
                $qCO = mysqli_query($db_conn, "SELECT id FROM `user_voucher_ownership` WHERE userid='$token->phone' AND voucherid='$id_voucher_redeemable'");
                if (mysqli_num_rows($qCO) >= $maxPerUser) {
                    $success =0;
                    $status =200;
                    $msg = "Kuota Anda Untuk Redeem Kode Ini Telah Habis";
                }else{
                    $res = mysqli_fetch_all($q, MYSQLI_ASSOC);
