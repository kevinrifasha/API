...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    $nama = mysqli_real_escape_string($db_conn, $obj['nama']);
    $desc = mysqli_real_escape_string($db_conn, $obj['Deskripsi']);
    ...
    if(isset($obj['nama']) && !empty($obj['nama'])){
        ...
        $menu = new Menu(array("nama"=>$nama,"sku"=>$obj['sku'],"id_partner"=>$obj['id_partner'],"harga"=>$obj['harga'],"Deskripsi"=>$desc,"id_category"=>$obj['id_category'],"category"=>"","img_data"=>$obj['img_data'],"enabled"=>$obj['enabled'],"stock"=>$obj['stock'],"is_variant"=>$obj['is_variant'],"hpp"=>$obj['hpp'],"harga_diskon"=>$obj['harga_diskon'],"is_recommended"=>0,"is_recipe"=>$obj['is_recipe'],"thumbnail"=>$obj['thumbnail'],"is_auto_cogs"=>$obj['is_auto_cogs'],"show_in_sf"=>$obj['show_in_sf'], "show_in_waiter"=>$obj['show_in_waiter']));
        ...
        if($obj['is_recipe']=='1' && $insert!=false || $obj['is_recipe']==1 && $insert!=false){
            foreach ($obj['recipes'] as $value) {
                ...
                $recipes = new Recipe(array("id_menu"=>$insert,"id_raw"=>$value['id_raw'],"qty"=>$value['qty'],"id_metric"=>$value['id_metric'],"id_variant"=>0,"id_partner"=>$partnerID));
        ...
        $getLastMenuID = mysqli_query($db_conn, "SELECT MAX(m.id) AS menu_id FROM `menu` m WHERE m.id_partner='$idPartner' ORDER BY `id` DESC");
