...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    if($newDateFormat == 1){
        if($all !== "1") {
            ...
            $query = "SELECT SUM(count) as count FROM ( SELECT COUNT(transaksi.id) AS count FROM transaksi JOIN partner p ON p.id = transaksi.id_partner WHERE p.id_master = '$idMaster' AND transaksi.deleted_at IS NULL AND (transaksi.status=2 OR transaksi.status=1 ) AND transaksi.paid_date BETWEEN '$dateFrom' AND '$dateTo' ";
        }
        ...
        if($all == "1") {
            $query = "SELECT SUM(count) as count, SUM(count2) as count2 FROM ( SELECT SUM(detail_transaksi.qty) AS count, COUNT(DISTINCT transaksi.id) AS count2 FROM detail_transaksi JOIN transaksi ON detail_transaksi.id_transaksi=transaksi.id JOIN partner p ON p.id = transaksi.id_partner WHERE detail_transaksi.is_smart_waiter=1 AND p.id_master = '$idMaster' AND transaksi.deleted_at IS NULL AND (transaksi.status=2 OR transaksi.status=1 ) AND transaksi.paid_date BETWEEN '$dateFrom' AND '$dateTo' ";
        } else {
        ...
        if($all == "1") {
            $query = "SELECT SUM(count) AS count FROM ( SELECT SUM(detail_transaksi.qty) AS count FROM detail_transaksi JOIN transaksi ON detail_transaksi.id_transaksi=transaksi.id JOIN partner p ON p.id = transaksi.id_partner WHERE p.id_master = '430' AND transaksi.deleted_at IS NULL AND (transaksi.status=2 OR transaksi.status=1 ) AND transaksi.paid_date BETWEEN '$dateFrom' AND '$dateTo' ";
        } else {
    ...
    else 
    {
        if($all !== "1") {
            ...
            $query = "SELECT SUM(count) as count FROM ( SELECT COUNT(transaksi.id) AS count FROM transaksi JOIN partner p ON p.id = transaksi.id_partner WHERE p.id_master = '$idMaster' AND transaksi.deleted_at IS NULL AND (transaksi.status=2 OR transaksi.status=1 ) AND DATE(transaksi.paid_date) BETWEEN '$dateFrom' AND '$dateTo' ";
        }
