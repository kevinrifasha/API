...
$signupMsg = 'Failed'; 
    ...
    if(isset($data->id) && !empty($data->id)){
        ...
        if($partner!=false){
            ...
            if(isset($data->name) && !empty($data->name)){
                $partner->setName(str_replace("'","''",$data->name));
            }
            if(isset($data->address) && !empty($data->address)){
                $partner->setAddress(str_replace("'","''",$data->address));
            }
            ...
            if(isset($data->partner_type) && !empty($data->partner_type)){
                $partner->setPartner_type($data->partner_type);
                if($data->partner_type==3){
                    $partner->setIs_foodcourt(1);
                }
            }
            ...
            if(($update==true || $subcatUpdate==true) && $signupMsg!="Password Lama Salah"){
                $success=1;
                $signupMsg="Success";
                $status = 200;
            }else{
    ...
    $signupJson = json_encode(["msg"=>$signupMsg, "success"=>$success, "status"=>$status, "subcat"=>$data->subcategory]);  
