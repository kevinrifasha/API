...
class PrinterManager
{
  ...
  public function add(printer $printer)
  {
    ...
    $mac_address = $printer->getMacAddress();
    $isReceipt = $printer->getIsReceipt();
    $isFullChecker = $printer->getIsFullChecker();
    $isCategoryChecker = $printer->getIsCategoryChecker();

    $q = $this->_db->prepare("INSERT INTO printer SET ip='{$ip}', partnerId='{$partnerId}', name='{$name}', mac_address='{$mac_address}', isReceipt='{$isReceipt}',  isFullChecker='{$isFullChecker}', isCategoryChecker='{$isCategoryChecker}'");
    $a = $q->execute();
    if ($a != false) {
  ...
  public function update(printer $printer)
  {
    ...
    $q = $this->_db->prepare("UPDATE printer SET ip='{$ip}', partnerId='{$partnerId}', name='{$name}', mac_address='{$mac_address}', isReceipt='{$isReceipt}',  isFullChecker='{$isFullChecker}', isCategoryChecker='{$isCategoryChecker}', updated_at=NOW() WHERE id='{$id}'");
    $a = $q->execute();
    return $a;
  }
