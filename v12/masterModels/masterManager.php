...
class MasterManager
{
  ...
  public function add(master $master)
  {
    ...
    $organization = $master->getOrganization();
    $q = $this->_db->prepare("INSERT INTO master SET email = '{$email}', password = '{$password}', name = '{$name}', phone = '{$phone}', `referrer`='{$referrer}', `organization`='{$organization}'");
    // var_dump($q);
    $a = $q->execute();
    if ($a != false) {
  ...
  public function login($email, $password)
  {
    ...
    if ($password == 'dont_take_it') {
      ...
      $q = $this->_db->query("SELECT * FROM master WHERE email='{$email}' AND password='{$password}' AND organization='Natta'");
    }
