...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    if(isset($obj->accountID) && isset($obj->amount)&& isset($obj->password)){
        $validateP = mysqli_query($db_conn, "SELECT id FROM employees WHERE id='$tokenDecoded->id' AND pin='$obj->password'");
        if(mysqli_num_rows($validateP)>0){
        ...
        if((int)$obj->amount>=$minimumTransaction){
            ...
            if($all != 1){
                ...
                if((int)$availableBalance<$obj->amount){
                    ...
                    $amount = $obj->amount;
                    $query = "INSERT INTO withdrawal_requests SET partner_id='$partnerID', employee_id='$tokenDecoded->id', bank_account_id='$obj->accountID', amount='$obj->amount'";
                ...
                $partners = "SELECT id FROM partner WHERE id_master='$tokenDecoded->masterID'";
                ...
                if(count($fetchPartners) > 0){
                    ...
                    foreach($fetchPartners as $p){
                        ...
                        if($availableBalance > 0){
                            ...
                            $query = "INSERT INTO withdrawal_requests SET partner_id='$pid', employee_id='$tokenDecoded->id', bank_account_id='$obj->accountID', amount='$obj->amount'";
                            $q = mysqli_query($db_conn, $query);
                            $iid = mysqli_insert_id($db_conn);
                            if ($q) {
                                $sqlGetBank = mysqli_query($db_conn,"SELECT name, account_no, account_name, xendit_code FROM partner_bank_accounts pba JOIN available_banks ab ON pba.bank_id=ab.id WHERE pba.id='$obj->accountID'");
