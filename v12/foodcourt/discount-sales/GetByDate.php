...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    if($newDateFormat == 1)
    {
        ...
        FROM ( SELECT special_discount, employee_discount, program_discount 
        FROM transaksi JOIN detail_transaksi ON transaksi.id=detail_transaksi.transaksi_id JOIN menu ON menu.id=detail_transaksi.id_menu
        WHERE menu='$id' AND (status=1 OR status=2 ) AND transaksi.deleted_at IS NULL AND transaksi.organization = 'Natta' AND transaksi.paid_date BETWEEN '$dateFrom' AND '$dateTo' GROUP BY transaksi.id ";
        $dateFromStr = str_replace("-","", $dateFrom);
        $dateToStr = str_replace("-","", $dateTo);
        $queryTrans = "SELECT table_name FROM information_schema.tables
        WHERE table_schema = '".$_ENV['DB_NAME']."' AND table_name LIKE 'transactions_%'";
    ...
    else
    {
        ...
        FROM ( SELECT special_discount, employee_discount, program_discount 
        FROM transaksi JOIN detail_transaksi ON transaksi.id=detail_transaksi.transaksi_id JOIN menu ON menu.id=detail_transaksi.id_menu
        WHERE menu='$id' AND (status=1 OR status=2 ) AND transaksi.deleted_at IS NULL AND transaksi.organization = 'Natta' AND DATE(transaksi.paid_date) BETWEEN '$dateFrom' AND '$dateTo' GROUP BY transaksi.id ";
        $dateFromStr = str_replace("-","", $dateFrom);
        $dateToStr = str_replace("-","", $dateTo);
        $queryTrans = "SELECT table_name FROM information_schema.tables
        WHERE table_schema = '".$_ENV['DB_NAME']."' AND table_name LIKE 'transactions_%'";
