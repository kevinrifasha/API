    ...
    $status = $tokens['status'];
        ...
        if($newDateFormat == 1){
            ...
            && isset($load) &&!empty($load)
                ...
                $history = $Tmanager->getByTenantIdWithHour($partnerId, $from, $to, $type);
            }
            $i = 0;
            ...
            foreach ($history as $val) {
                ...
                $idT = $val['id'];
                $statusT = $val['status'];
                $sql = mysqli_query($db_conn, "SELECT DATE_ADD(order_status_trackings.created_at, INTERVAL 7 HOUR) as created_at FROM `order_status_trackings` WHERE `transaction_id` LIKE '$idT' AND `status_after` = '$statusT' ORDER BY `id`  DESC");
        ...
        else 
        {
            ...
            && isset($load) &&!empty($load)
                ...
                $history = $Tmanager->getByTenantId($partnerId, $from, $to, $type);
            }
            $i = 0;
