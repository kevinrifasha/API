...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    FROM `transaksi`
    JOIN partner as parent ON parent.id=transaksi.id_partner
    JOIN partner ON partner.fc_parent_id=parent.id
    LEFT JOIN
            (
    ...
    while($row=mysqli_fetch_assoc($transaksi)){
        ...
        FROM `$transactions`
        JOIN partner as parent ON parent.id=`$transactions`.id_partner
        JOIN partner ON partner.fc_parent_id=parent.id
        LEFT JOIN
                (
                    SELECT delivery.transaksi_id as did
