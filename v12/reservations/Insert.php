...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    $q = mysqli_query($db_conn, "INSERT INTO `reservations`(phone, email, `partner_id`, `user_id`, `for_reservation_id`, `name`, `description`, `persons`, `minimum_transaction`, `booking_price`, `duration`, `duration_metric`, `reservation_time`, `end_time`, `status`, partner_notes, table_id) VALUES ( '$data->phone','$data->email','$data->partner_id', '$data->user_id', '$data->for_reservation_id', '$data->name', '$data->description', '$data->capacity', 0, 0, '$data->duration', 'Hour', '$data->reservation_time', '$data->end_time', 'Approved', '$data->partner_notes', '$data->table_id')");
    if ($q) {
        $success = 1;
