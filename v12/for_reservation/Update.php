...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    if($isOverlapped > 0 && $countSchedules > 1) {
        ...
        && isset($data->name) && !empty($data->name)
                ...
                $update = mysqli_query($db_conn,"UPDATE `for_reservation` SET `partner_id`='$data->partner_id',`meja_id`='$data->meja_id',`table_group_id`='$data->table_group_id',`name`='$data->name',`description`='$data->description',`capacity`='$data->capacity',`minimum_transaction`='$data->minimum_transaction',`booking_price`='$data->booking_price',`duration`='$data->duration',`duration_metric`='$data->duration_metric', `image`='$data->image', `updated_at`=NOW() WHERE `id`='$data->id'");
                if($update){
