    ...
    if($newDateFormat == 1){
        ...
        if(mysqli_num_rows($sql) > 0) {
            while ($row = mysqli_fetch_assoc($sql)) {
                ...
                $result['totalBeforeTax'] = $totalBeforeTax;
                $result['taxPercentage'] = $avgTax;
            ...
            $result = array_reduce($array, function($carry, $item){ 
                if(!isset($carry[$item['id_partner']])){ 
                    ...
                    [
                        ...
                        'total_tax_percentage'=>$item['taxPercentage'],
                        'totalTrx'=>1,
                    ]; 
                } else { 
                    $carry[$item['id_partner']]['tax'] += $item['tax']; 
                    $carry[$item['id_partner']]['totalBeforeTax'] += $item['totalBeforeTax'];
                    $carry[$item['id_partner']]['total_tax_percentage'] += $item['taxPercentage'];
            ...
            foreach($result as $val) {
                $val['tax_percentage'] = $val['total_tax_percentage'] / $val['totalTrx'];
                unset($val['total_tax_percentage']);
                unset($val['totalTrx']);
