...
if (isset($tokenValidate['success']) && $tokenValidate['success'] == 0) {
    ...
    if (
        ...
        if (isset($obj->detail_id) && !empty($obj->detail_id)) {
            $q = mysqli_query($db_conn, "SELECT id, id_partner, charge_ur, status, employee_discount_percent, id_voucher FROM `transaksi` WHERE id='$obj->transaction_id'  AND diskon_spesial='0' AND program_discount='0' AND organization='Natta'");
            $partner_id = '';
            $charge_ur = 0;
            $total = 0;
            $status = 0;
