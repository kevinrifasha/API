...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
  ...
  if($newDateFormat == 1){
    if(isset($_GET['all']) && $all == "1"){
        ...
        $employees = mysqli_query($db_conn, "SELECT k.id, k.nama AS name FROM employees k LEFT JOIN partner p ON p.id='$partner_id' WHERE k.deleted_at IS NULL AND p.id_master='$master_id' AND k.organization='Natta'");
    }else{
        $employees = mysqli_query($db_conn, "SELECT k.id, k.nama AS name FROM employees k WHERE k.deleted_at IS NULL AND k.id_partner='$partner_id' AND k.organization='Natta'");
    }
