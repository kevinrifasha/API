...
$data = json_decode(json_encode($_POST));
    ...
    $status = $tokens['status'];
            ...
            $transactionDetails = $TDmanager->getDetailById($id);
            $transactionDetails->setStatus($status);
            $TDmanager->update($transactionDetails);
            //transaction
            $Tmanager = new TransactionManager($db);
            $transaction = $Tmanager->getTransaction($transactionDetails->getId_transaksi());
            ...
            //Detail Transaction
            $TDmanager1 = new TransactionDetailManager($db);
            $transactionDetails = $TDmanager1->getDetail($transaction->getId());
            $validator = true;
