...
//                 WHERE DATE(transaksi.paid_date) BETWEEN '$dateFrom' AND '$dateTo' AND transaksi.id_partner='$id' AND transaksi.deleted_at IS NULL AND DATE(jam) BETWEEN '$dateFrom' AND '$dateTo' AND transaksi.pre_order_id !=0 AND (transaksi.status='1' OR transaksi.status='2' ) AND transaksi.deleted_at IS NULL ";
...
//                     WHERE DATE(`$transactions`.paid_date) BETWEEN '$dateFrom' AND '$dateTo' AND 
//                     `$transactions`.id_partner='$id' AND `$transactions`.deleted_at IS NULL AND DATE(jam) BETWEEN '$dateFrom' AND '$dateTo' AND `$transactions`.pre_order_id !=0 AND (`$transactions`.status='1' OR `$transactions`.status='2' ) AND `$transactions`.deleted_at IS NULL ";
//                 }
//             }
//             $sqlCountPreorder = mysqli_query($db_conn, $query);
            
//             $query="SELECT COUNT(d.id) AS delivery FROM delivery d JOIN transaksi t ON t.id= d.transaksi_id WHERE t.id_partner='$id' AND d.deleted_at IS NULL AND DATE(t.jam) BETWEEN '$dateFrom' AND '$dateTo' AND (t.status='1' OR t.status='2') AND t.deleted_at IS NULL ";
...
//                 if(($dateFromStr>=$table_name[1]) || ($dateToStr>=$table_name[2])){
//                     $query .= "UNION ALL " ;
//                     $query .= "SELECT COUNT(d.id) AS delivery FROM delivery d JOIN `$transactions` t ON t.id= d.transaksi_id WHERE t.id_partner='$id' AND d.deleted_at IS NULL AND DATE(t.jam) BETWEEN '$dateFrom' AND '$dateTo' AND (t.status='1' OR t.status='2') AND t.deleted_at IS NULL ";
//                 }
//             }
...
//                 $partner['dineIn'] = $dineIn[0]['dineIn'];
//                 $partner['takeaway'] = $takeaway[0]['takeaway'];
//                 $partner['preorder'] = $preorder[0]['preorder'];
//                 $partner['delivery'] = $delivery[0]['delivery'];
//                 $partner['total'] = $partner['dineIn'] + $partner['takeaway'] +  $partner['preorder'] + $partner['delivery'];
    ...
    if($newDateFormat == 1)
    {
        ...
        if(mysqli_num_rows($sqlPartner) > 0) {
            ...
            foreach($getPartners as $partner) {
                ...
                $query="SELECT COUNT(transaksi.id) AS preorder FROM transaksi 
                    WHERE transaksi.paid_date BETWEEN '$dateFrom' AND '$dateTo' AND transaksi.id_partner='$id' AND transaksi.deleted_at IS NULL AND jam BETWEEN '$dateFrom' AND '$dateTo' AND transaksi.pre_order_id !=0 AND (transaksi.status='1' OR transaksi.status='2' ) AND transaksi.deleted_at IS NULL ";
