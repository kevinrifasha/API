...
//     } else {
//         $query = "SELECT detail_transaksi.variant, detail_transaksi.id_transaksi, menu.nama, menu.id AS id_menu, menu.harga AS harga_satuan, transaksi.status, detail_transaksi.qty, detail_transaksi.id AS det_transc_id, partner.id AS id_partner FROM menu join detail_transaksi on menu.id=detail_transaksi.id_menu join transaksi on detail_transaksi.id_transaksi = transaksi.id join partner on transaksi.id_partner=partner.id WHERE partner.id_master= '$idMaster' AND transaksi.status IN (1,2,3,4) AND DATE(transaksi.paid_date) BETWEEN '$dateFrom' AND '$dateTo' ORDER BY detail_transaksi.id_transaksi DESC";
//     }
...
//                         $reportV[$v_id]['price'] = (int)$variantPrice;
//                         $reportV[$v_id]['gross_sales'] = $reportV[$v_id]['price'] * $reportV[$v_id]['qty'];
//                         $reportV[$v_id]['gross_sales'] = round($reportV[$v_id]['gross_sales'], 2);
