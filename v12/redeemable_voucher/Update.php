...
if(isset($tokens['success']) && $tokens['success']=='0' || isset($tokens['success']) && $tokens['success']==0){
    ...
    if(
        ...
        $prerequisite=json_encode($obj['prerequisite']);
            $sqlInsert = "UPDATE redeemable_voucher SET code='{$code}', title='{$title}', description='{$description}', type_id='{$type_id}', is_percent='{$is_percent}', discount='{$discount}', enabled='{$enabled}', valid_from='{$valid_from}', valid_until='{$valid_until}', total_usage='{$total_usage}', img='{$img}', created_at= NOW(), prerequisite='{$prerequisite}'";
            ...
            if(isset($obj['partner_id']) && !empty($obj['partner_id'])){
                ...
                $sqlInsert = $sqlInsert.",partner_id=null";
            }
