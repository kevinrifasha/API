...
<body>
	...
	?>
 	<center>
        ...
        <br/>
        <b>Periode: <?php echo $fs->tgl_indo(date('m', strtotime($dateEnd)));?><b>
	</center>
    <?php
    $attendance = mysqli_query($db_conn, "SELECT a.id, a.employee_id, a.in_time, a.out_time, a.in_image, a.out_image, k.nama as name, DATE(a.in_time) as date,HOUR(a.in_time) as in_hour, MINUTE(a.in_time) as in_minute, SECOND(a.in_time) as in_second, HOUR(a.out_time) as out_hour, MINUTE(a.out_time) as out_minute, SECOND(a.out_time) as out_second, a.schedule_in_time, a.schedule_out_time  FROM attendance a JOIN employees k ON k.id=a.employee_id WHERE k.id_partner='$fID' AND DATE(a.in_time) BETWEEN '$dateStartNowDb' AND '$dateEndNowDb' ORDER BY DATE(a.in_time) DESC  ");
