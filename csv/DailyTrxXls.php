...
<body>
	...
	?>
 	<center>
        <h1>Laporan Penjualan <br><?php echo $name ?></h1>
  ...
  <br/>
    <b>Rincian Jenis Pembayaran<b>
      ...
      <?php
      $vals = $cf->getGroupPaymentMethod($fID, $dateNowDb, $dateNowDb);
      $sales = 0;
      $charge_ur = 0;
      $charge_ewallet = 0;
      $total = 0;
  ...
  <b>Laba Kotor<b>
    ...
    $sql = mysqli_query($db_conn, "SELECT SUM(op.amount) as amount FROM operational_expenses op JOIN operational_expense_categories opc ON op.category_id=opc.id JOIN partner p ON p.id_master=opc.master_id JOIN employees e ON e.id=op.created_by WHERE p.id='$fID'AND op.deleted_at IS NULL AND DATE(op.created_at) BETWEEN '$dateNowDb' AND '$dateNowDb' ORDER BY op.id DESC");
    $res['operational']=0;
