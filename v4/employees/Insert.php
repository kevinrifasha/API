...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if(
        ...
        if($data->email !== "") {
            $email = $data->email;
            $validateEmail = mysqli_query($db_conn, "SELECT id FROM employees WHERE email='$email' AND deleted_at IS NULL");

            if(mysqli_num_rows($validateEmail)>0){
                $msg = "Terdapat Email duplikat. Mohon gunakan Email lain";
                $success=0;
                $status=204;
            }
                ...
                $q = "INSERT INTO `employees` SET `nama`='$data->name', id_master='$token->id_master', id_partner='$token->id_partner', email='$data->email', phone='$data->phone', nik='$data->nik', pin='$pin', role_id='$data->roleID', pattern_id='$data->patternID', show_as_server=1";
            }
        } if($data-nik !== ""){
                $nik = $data->nik;
                $validateNik = mysqli_query($db_conn,"SELECT id FROM employees WHERE nik='$nik' AND deleted_at IS NULL");
