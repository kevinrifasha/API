    ...
    if(mysqli_num_rows($getPartner) > 0){
        ...
        foreach($getPartner as $val){
            ...
            if(mysqli_num_rows($transaksi)>0){
                ...
                //     $daily[$j]['value']=ceil($row['total'])-ceil($row['promo'])-ceil($row['program_discount'])-ceil($row['diskon_spesial'])-ceil($row['employee_discount'])-ceil($row['point'])+ceil($row['service'])+ceil($row['tax'])+ceil($row['charge_ur']);
                //     $daily[$j]['date'] = DATE('d-m-Y',strtotime($row['created_at']));
                //     $j+=1;
                // }
            } else {
                array_push($daily, array("date" => "Tidak Ada Data" , "value" => "0" ));
            }
            
            $arrayDone["id_partner"] = $id;
            $arrayDone["name"] = $name;
            $arrayDone["dailyIncome"] = $daily;
