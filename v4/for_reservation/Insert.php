...
//     $tableCount = count($data->table_id);
//     $isError = false;
...
//           $insert = mysqli_query($db_conn,"INSERT INTO `for_reservation`(`partner_id`, `meja_id`, `table_group_id`, `name`, `description`, `capacity`, `minimum_transaction`, `booking_price`, `duration`, `duration_metric`, `image`) VALUES ('$data->partner_id', '$value', '$data->table_group_id', '$data->name', '$data->description', '$data->capacity', '$data->minimum_transaction', '$data->booking_price', '$data->duration', '$data->duration_metric', '$data->image')");
//           if($insert) {
...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
  ...
  // POST DATA
    ...
    if($isOverlapped > 0 && $countSchedules > 1) {
      ...
      $tables = $data->table_id;
      if(($data->table_group_id != "0" || $data->table_group_id != 0) && !is_array($tables)) {
        //   $queryTableID = "SELECT m.id FROM meja m LEFT JOIN table_group_details tgd ON tgd.table_id = m.id WHERE tgd.table_group_id = '$data->table_group_id' AND tgd.deleted_at IS NULL";
        
          $insertQuery = mysqli_query($db_conn,"INSERT INTO `for_reservation`(`partner_id`, `meja_id`, `table_group_id`, `name`, `description`, `capacity`, `minimum_transaction`, `booking_price`, `duration`, `duration_metric`, `image`) VALUES ('$data->partner_id', '$data->table_id', '$data->table_group_id', '$data->name', '$data->description', '$data->capacity', '$data->minimum_transaction', '$data->booking_price', '$data->duration', '$data->duration_metric', '$data->image')");
        
        //   $mqTableID = mysqli_query($db_conn, $queryTableID);
        //   $mqFetchAll = mysqli_fetch_all($mqTableID, MYSQLI_ASSOC);
          
        //   if(count($mqFetchAll) > 0){
        //       $tables = array_values($mqFetchAll); 
        //   }
      }
