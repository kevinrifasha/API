...
if (isset($tokenValidate['success']) && $tokenValidate['success'] == 0) {
    ...
    // $query =  "SELECT id, jam, phone, no_meja, status, total, id_voucher, id_voucher_redeemable, tipe_bayar, promo, diskon_spesial, employee_discount, point, queue, takeaway, notes, tax, service, charge_ur, payment_method, uname, program_discount, employee_discount_percent FROM ( SELECT t.id, t.jam, t.phone, t.no_meja, t.status, t.total, t.id_voucher, t.id_voucher_redeemable, t.tipe_bayar, t.promo, t.diskon_spesial, t.employee_discount, t.point, t.queue, t.takeaway, t.notes, t.tax, t.service, t.charge_ur, pm.nama as payment_method, case when u.name is null or t.is_helper=1 or t.is_pos then t.customer_name else u.name end AS uname, t.program_discount, t.employee_discount_percent FROM transaksi t JOIN payment_method pm ON t.tipe_bayar = pm.id LEFT JOIN users u ON u.phone=t.phone WHERE t.id_partner='$token->id_partner' AND t.status IN(2,7) AND t.group_id = '$groupID' ";

    // t.status IN(2,7) di ganti dengan t.status = '5' karena kan yang pending itu statusnya 5
    $query =  "SELECT id, jam, phone, no_meja, status, total, id_voucher, id_voucher_redeemable, tipe_bayar, promo, diskon_spesial, employee_discount, point, queue, takeaway, notes, tax, service, charge_ur, payment_method, uname, program_discount, employee_discount_percent, rounding FROM ( SELECT t.id, t.jam, t.phone, t.no_meja, t.status, t.total, t.id_voucher, t.id_voucher_redeemable, t.tipe_bayar, t.promo, t.diskon_spesial, t.employee_discount, t.point, t.queue, t.takeaway, t.notes, t.tax, t.service, t.charge_ur, pm.nama as payment_method, case when u.name is null or t.is_helper=1 or t.is_pos then t.customer_name else u.name end AS uname, t.program_discount, t.employee_discount_percent, t.rounding FROM transaksi t JOIN payment_method pm ON t.tipe_bayar = pm.id LEFT JOIN users u ON u.phone=t.phone WHERE t.id_partner='$token->id_partner' AND t.status != '5' AND t.group_id = '$groupID' ";
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        foreach ($res as $value) {
            ...
            $query =  "SELECT status, partner_note, takeaway, pmName, total, charge_ur, service, tax, diskon_spesial, employee_discount, promo, program_discount, rounding FROM ( SELECT t.status, t.partner_note, t.takeaway, pm.nama AS pmName, t.total, t.charge_ur, t.service, t.tax, t.diskon_spesial, t.employee_discount, t.promo, t.program_discount, t.rounding FROM transaksi t JOIN payment_method pm ON t.tipe_bayar = pm.id WHERE t.id='$transactionID' ";
            ...
            // $query =  "SELECT id, id_menu, harga_satuan, qty, notes, harga, status, nama, variant, cName, category_id FROM ( SELECT dt.id, dt.id_menu, dt.harga_satuan, dt.qty, dt.notes, dt.harga, dt.status, m.nama, dt.variant, c.name AS cName, c.id AS category_id FROM detail_transaksi dt JOIN menu m ON dt.id_menu=m.id JOIN categories c ON m.id_category=c.id WHERE dt.id_transaksi='$transactionID' ";
