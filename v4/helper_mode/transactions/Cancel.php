    ...
    if(
        ...
        foreach ($trasactionID as $value) {
            ...
            if (mysqli_num_rows($qT) > 0) {
                ...
                $tokensQ = mysqli_query($db_conn, "SELECT tokens FROM `device_tokens` WHERE user_phone='$phone' AND deleted_at ='0000-00-00 00:00:00' OR user_phone='$phone' AND deleted_at IS NULL");
                $tokensQP = mysqli_query($db_conn, "SELECT tokens FROM `device_tokens` WHERE id_partner='$id_partner' AND deleted_at ='0000-00-00 00:00:00' OR id_partner='$id_partner' AND deleted_at IS NULL");
                $udevTokens = array();
                $devTokens = array();
                $i=0;
                $j=0;
                ...
                $insertOST = mysqli_query($db_conn, "INSERT INTO `order_status_trackings`(`transaction_id`, `status_before`, `status_after`, `created_at`,`created_by`) VALUES ('$id', '$tStatus', '$obj->status', NOW(), '$token->id')");
