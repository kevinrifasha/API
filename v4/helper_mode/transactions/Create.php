...
function insertVoucherRedeem($phone, $vr, $trx ,$db_conn){
    if($phone != "" && $vr != ""){
        $insert = mysqli_query($db_conn,"INSERT INTO `user_voucher_ownership` SET `transaksi_id`='$trx', `userid`='$phone', `voucherid`='$vr', `obtained`='1'");
    }
}
        ...
        && !empty($data->paymentMethod) ){
            ...
            if($boolQty==true){
                ...
                if($paymentMethod=='11' || $paymentMethod==11 || $paymentMethod=='1' || $paymentMethod==1 || $paymentMethod=='2' || $paymentMethod==2 || $paymentMethod=='3' || $paymentMethod==3 || $paymentMethod=='4' || $paymentMethod==4 || $paymentMethod=='6' || $paymentMethod==6 || $paymentMethod=='10' || $paymentMethod==10){
                    $sql .= "INSERT INTO transaksi(`id`, `jam`, `phone`, `customer_name`,  `customer_email`, `id_partner`, `no_meja`, `status`, `total`, `tipe_bayar`, `promo`, `point`, `queue`, `takeaway`, `notes`, `id_foodcourt`, `tax`, `service`, `charge_ewallet`, `charge_xendit`, `charge_ur`, `id_voucher`, `id_voucher_redeemable`, `diskon_spesial`, `shift_id`, `program_discount`, `program_id`,`is_helper`) VALUES ('$id', '$today', '$phone', '$customer_name', '$customer_email', '$partnerID', '$tableCode', '$status', '$total', '$paymentMethod', '$promo', '$point', '$is_queue', '$is_takeaway', '$notes', '$foodcourtID', '$tax', '$service', '$charge_ewallet', '$charge_xendit', '$charge_ur', '$id_voucher', '$id_voucher_redeemable', '$diskon_spesial', '$shiftID', '$program_discount', '$program_id','1'); ";
                }else{
                    $sql .= "INSERT INTO transaksi(`id`, `jam`, `phone`, `customer_name`, `customer_email`, `id_partner`, `no_meja`, `status`, `total`, `tipe_bayar`, `promo`, `point`, `queue`, `takeaway`, `notes`, `id_foodcourt`, `tax`, `service`, `charge_ewallet`, `charge_xendit`, `charge_ur`, `id_voucher`, `id_voucher_redeemable`, `diskon_spesial`, `program_discount`, `program_id`,`is_helper`) VALUES ('$id', '$today', '$phone', '$customer_name', '$customer_email', '$partnerID', '$tableCode', '$status', '$total', '$paymentMethod', '$promo', '$point', '$is_queue', '$is_takeaway', '$notes', '$foodcourtID', '$tax', '$service', '$charge_ewallet', '$charge_xendit', '$charge_ur', '$id_voucher', '$id_voucher_redeemable', '$diskon_spesial', '$program_discount', '$program_id','1'); ";
                }
                if($is_delivery=='1' || $is_delivery==1){
                    if (strpos($delivery_detail, 'Kurir Pribadi') !== false) {
                        // echo 'true';
                ...
                // if($insert){
                    ...
                    insertVoucherRedeem($phone, $id_voucher_redeemable, $id ,$db_conn);
                    $dataDetail = $data->detail;
                    $idx = 0;
                    $il=0;
                    $items = array();
