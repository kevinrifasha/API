...
if (isset($tokens['success']) && $tokens['success'] == '0' || isset($tokens['success']) && $tokens['success'] == 0) {
    ...
    if (isset($_GET['employeeID']) && !empty($_GET['employeeID'])) {
        ...
        // $addtionalQuery = "AND employee_id LIKE '%$employeeID%'";
    }
    ...
    if (
        ...
        if ($all == "1") {
            $query = "SELECT s.id, s.start, s.end, s.petty_cash, s.employee_id, s.actual_cash FROM shift s JOIN partner p ON p.id = s.partner_id WHERE p.id_master='$idMaster' AND DATE(s.created_at) BETWEEN '$dateFrom' AND '$dateTo' AND s.deleted_at IS NULL " . $addtionalQuery . " ORDER BY id DESC";
        } else {
        ...
        if ($all == "1") {
            $query = "SELECT s.id, s.start, s.end, s.petty_cash, s.employee_id FROM shift s JOIN partner p ON p.id = s.partner_id WHERE p.id_master = '$idMaster' AND s.deleted_at IS NULL " . $addtionalQuery . " ORDER BY id DESC";
        } else {
