...
if(
    ...
    $q = mysqli_query($db_conn, "SELECT employees.id,employees.nama AS name,employees.id_master,employees.id_partner,employees.role_id, partner.name as master_name, partner.address, r.id as role_id, r.name as role_name, r.is_owner,employees.nik, employees.gender, employees.email, employees.phone, partner.is_attendance,partner.is_reservation, partner.max_people_reservation, partner.max_days_reservation, partner.is_table_management, partner.is_foodcourt, parent.id AS parentID, partner.partner_type, parent.name AS parentName, parent.is_foodcourt AS parentFC, CASE WHEN partner.fc_parent_id !='0' THEN parent.is_centralized WHEN partner.fc_parent_id ='0' THEN partner.is_centralized ELSE 0 END AS is_centralized, CASE WHEN partner.fc_parent_id !='0' THEN parent.delivery_status_tracking WHEN partner.fc_parent_id ='0' THEN partner.delivery_status_tracking ELSE 0 END AS delivery_status_tracking FROM employees LEFT JOIN master ON master.id=employees.id_master JOIN partner ON employees.id_partner=partner.id JOIN roles r ON employees.role_id=r.id LEFT JOIN partner AS parent on partner.fc_parent_id = parent.id WHERE employees.email='$email' AND employees.pin='$password'");
