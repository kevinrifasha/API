            ...
            if (mysqli_num_rows($qv) > 0) {
                ...
                foreach ($data['variants'] as $value1) {
                    ...
                    if($value1['is_recipe']=='1'){
                        $arr=array();
                        $qR = mysqli_query($db_conn, "SELECT r.id, r.id_raw, r.qty, r.id_metric, rw.name as name_raw, m.name as name_metric FROM recipe r JOIN raw_material rw ON rw.id=r.id_raw JOIN metric m ON m.id=r.id_metric WHERE r.id_variant='$finRW' AND r.deleted_at IS NULL");
