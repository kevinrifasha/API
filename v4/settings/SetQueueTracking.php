...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    $id = $token->id_partner;
    $query = "";
    if(isset($data->is_queue_tracking)){
        $query = "UPDATE partner SET is_queue_tracking='$data->is_queue_tracking' WHERE id='$id'";
        $sql = mysqli_query($db_conn, $query);
     if($sql){
         $success = 1;
         $status = 200;
         if($data->is_queue_tracking=="1"||$data->is_queue_tracking==1){
