...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if($is_owner == '1'){
        ...
        $addQuery3 = "AND d.id IN (" . implode(',', $arrPermission) . ")";
    }
    
    $qDepts = mysqli_query($db_conn,"SELECT d.id, d.name, d.partner_id FROM departments d ". $addQuery2 ." WHERE ". $addQuery1 ." ".$addQuery3." AND d.deleted_at IS NULL");
...
echo json_encode(["success"=>$success, "status"=>$status,"msg"=>$msg, "breakdowns"=>$data, "data"=>$depts, "test"=>$arrPermission]);
