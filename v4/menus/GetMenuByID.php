    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        foreach ($res as $value) {
            ...
            if ($value['is_recipe'] == 1) {
                ...
                $qv = mysqli_query($db_conn, "SELECT r.id, r.id_menu, r.id_raw, r.qty, r.id_metric, r.id_variant, m.nama AS menuName, m.img_data AS menuImage, rw.name AS rawName, me.name AS metricName 
                              FROM recipe r 
                              JOIN menu m ON m.id=r.id_menu 
                              JOIN raw_material rw ON rw.id=r.id_raw AND rw.deleted_at IS NULL
                              JOIN metric me ON me.id=r.id_metric 
                              WHERE r.id_menu='$menu_id' AND r.deleted_at IS NULL");
