...
if ($event == "qr.payment") {
    ...
    if (mysqli_num_rows($getTrx) < 1) {
        $getTrx = mysqli_query(
            $db_conn,
            "SELECT transaksi.id ,transaksi.pre_order_id, transaksi.no_meja, transaksi.id_partner, meja.is_queue, transaksi.takeaway, transaksi.phone, transaksi.jam, transaksi.total, transaksi.id_voucher, transaksi.id_voucher_redeemable, transaksi.tipe_bayar, transaksi.promo, transaksi.diskon_spesial, transaksi.point, transaksi.queue, transaksi.notes, transaksi.tax, transaksi.service, transaksi.charge_ur, customer_name AS uname, payment_method.nama AS payment_method, transaksi.status, employee_discount FROM `transaksi` JOIN partner ON transaksi.id_partner=partner.id JOIN payment_method ON payment_method.id=transaksi.tipe_bayar JOIN meja ON meja.idpartner=transaksi.id_partner AND meja.idmeja=transaksi.no_meja WHERE transaksi.id LIKE '%$external_id%' AND ((status_callback NOT LIKE '%SUCCEEDED%' AND status_callback NOT LIKE '%PAID%' AND status_callback NOT LIKE '%SETTLED%' AND status_callback NOT LIKE '%COMPLETED%') OR status_callback IS NULL) AND transaksi.deleted_at IS NULL ORDER BY transaksi.jam DESC LIMIT 1"
        );
    }
    ...
    $test4 = "SELECT transaksi.id ,transaksi.pre_order_id, transaksi.no_meja, transaksi.id_partner, meja.is_queue, transaksi.takeaway, transaksi.phone, transaksi.jam, transaksi.total, transaksi.id_voucher, transaksi.id_voucher_redeemable, transaksi.tipe_bayar, transaksi.promo, transaksi.diskon_spesial, transaksi.point, transaksi.queue, transaksi.notes, transaksi.tax, transaksi.service, transaksi.charge_ur, customer_name AS uname, payment_method.nama AS payment_method, transaksi.status, employee_discount FROM `transaksi` JOIN partner ON transaksi.id_partner=partner.id JOIN payment_method ON payment_method.id=transaksi.tipe_bayar JOIN meja ON meja.idpartner=transaksi.id_partner AND meja.idmeja=transaksi.no_meja WHERE transaksi.id LIKE '%$external_id%' AND ((status_callback NOT LIKE '%SUCCEEDED%' AND status_callback NOT LIKE '%PAID%' AND status_callback NOT LIKE '%SETTLED%' AND status_callback NOT LIKE '%COMPLETED%') OR status_callback IS NULL) AND transaksi.deleted_at IS NULL ORDER BY transaksi.jam DESC LIMIT 1";
    ...
    if (
        ...
        $status = 1;
        if ($tStatus == '5' || (int)$tStatus == 5){
            $status = 2;
        }
        $insertOST = mysqli_query(
            $db_conn,
            "INSERT INTO `order_status_trackings`(`transaction_id`, `status_before`, `status_after`, `payment_method_before`, `payment_method_after`) VALUES ('$external_id', '$tStatus', '$status', '$payment_method' , '14')"
        ); 
        ...
        if ($is_queue != 1 && $takeaway != 1 && $is_delivery != 1) {
            ...
            $updateTrans = mysqli_query(
                $db_conn,
                "UPDATE transaksi SET paid_date=NOW(), status='$status', queue='$var', shift_id='$lastShiftID', notes='$tmp', tipe_bayar='14' WHERE id='$external_id'"
            );
            if ($updateTrans) {
                $response["error"] = false;
        ...
        foreach ($devTokens as $val) {
            ...
            $message =
                "Pembayaran pesanan untuk meja " .
                $no_meja .
                " telah diterima";
            if ($takeaway == "1") {
                $message =
                    "Pembayaran pesanan untuk takeaway telah diterima";
            }
            if ($is_delivery == 1) {
                $message =
                    "Pembayaran pesanan untuk delivery telah diterima";
            }
            if ($pre_order_id == "1") {
                $message =
                    "Pembayaran pesanan untuk pre-order telah diterima";
            }
            $id = null;
            $action = null;
            $test2 = [$fcm_token,
        ...
        $UpdateCallback = mysqli_query(
            $db_conn,
            "UPDATE `transaksi` SET `status_callback`='$data1',`callback_hit`=`callback_hit`+1, `callback_at`=NOW(), shift_id='$lastShiftID', paid_date=NOW() WHERE id='$external_id'"
        );
    } else {
        $status = 0;
        if ($is_queue != 1 && $takeaway != 1) {
            $updateTrans = mysqli_query(
                $db_conn,
                "UPDATE transaksi SET paid_date=NOW(), shift_id='$lastShiftID', tipe_bayar='14' WHERE id='$id'"
            );
            if ($updateTrans) {
                $response["error"] = false;
                $response["message"] =
            ...
            $updateTrans = mysqli_query(
                $db_conn,
                "UPDATE transaksi SET paid_date=NOW(), queue='$var', shift_id='$lastShiftID', tipe_bayar='14' WHERE id='$id'"
            );
            if ($updateTrans) {
                $response["error"] = false;
                $response["message"] = "Transaksi Update Successfully";
        ...
        $insertOST = mysqli_query(
            $db_conn,
            "INSERT INTO `order_status_trackings`(`transaction_id`, `status_before`, `status_after`) VALUES ('$external_id', '$tStatus', '$status')"
        );
