...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if(
        ...
        $sql = mysqli_query($db_conn, "SELECT status FROM transaksi WHERE id = '$transactionID' AND deleted_at IS NULL");
        ...
        if($transcStatus == 0) {
            ...
            foreach ($trasactionID as $value) {
                if(!empty($value)){
                    ...
                    $open_bill = $trans[0]['open_bill'];
                    $shiftID="0";
                    if($open_bill == 1 && ($obj->paymentMethod=='11' || $obj->paymentMethod==11 )){
                        $status = 5;
...
echo json_encode(["success"=>$success, "status"=>$status, "msg"=>$msg, "newID"=>$newID, "ewallet_response"=>$ewallet_response]);
?>