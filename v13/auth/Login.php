...
)
{
    ...
    // $newPin = md5($pin);
    $q = mysqli_query($db_conn, "SELECT name FROM `users` WHERE  email='$email' AND organization='Natta'");
    // if(!empty($obj->email)){
    // }
    // else
    // {
    ...
    if (mysqli_num_rows($q) > 0) {
        // if(!empty($obj->email)){
        $q = mysqli_query($db_conn, "SELECT id, email, name, phone, pin FROM `users` WHERE  email='$email' AND (password ='$newPassword' OR pin='$newPassword') AND organization='Natta'"); 
        // }else{
        ...
        if (mysqli_num_rows($q) > 0) {
            ...
            $jsonToken = json_encode(['email'=>$fetched['email'], 'phone'=>$fetched['phone'], 'created_at'=>$today, 'expired'=>500, "id"=>$fetched['id'] , "email"=>$fetched['email'], "name"=>$fetched['name']]);
