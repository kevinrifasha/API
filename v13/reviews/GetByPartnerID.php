...
$rating = "SELECT IFNULL(SUM(r.rating),1) AS sum, IFNULL(COUNT(r.rating),1)  AS count FROM reviews r JOIN transaksi t ON t.id = r.transaction_id WHERE t.id_partner='$id' AND r.deleted_at IS NULL AND t.deleted_at IS NULL";
...
echo json_encode(["success"=>$success, "status"=>$status, "msg"=>$msg, "reviews"=>$res,"average"=>$average, "count"=>$count]);