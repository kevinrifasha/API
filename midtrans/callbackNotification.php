...
// echo($data['transaction_time']);
// var_dump($data);
$ewallet_type = $data['payment_type'];
$status = $data['transaction_status'];
$external_id = $data['order_id'];
// $amount = mysqli_real_escape_string($db_conn, trim($data['gross_amount']));
...
if (strpos($external_id, 'TOPUP') !== false || strpos($external_id, 'BILL') !== false) {
    ...
    if ($status == "SUCCEEDED" || $status == "PAID" || $status == "SETTLED" || $status == "COMPLETED" || $status == "completed" || $status == "paid" || $status == "settled") {
        ...
        foreach ($udevTokens as $val) {
            ...
            $fcm_token = $dev_token;
            $title = "Pesanan Diproses";
            $message = "Pesanan " . $type . " - " . "$operator" . " anda diproses.";
            $id = null;
            $action = null;
        }
    } else {
        ...
        foreach ($udevTokens as $val) {
            ...
            $fcm_token = $dev_token;
            $title = "Pembayaran Gagal";
            $message = "Pesanan " . $type . " - " . "$operator" . " anda dibatalkan.";
            $id = null;
            $action = null;
            ...
            $extraNotificationData = [
                "status" => $status, "event" => "ewallet.payment",  "soundAndroid" => "bell_new_order",
                "soundIos" => "bell_new_order", "message" => $message, "title" => $title, "id" => $id, "action" => $action, "id_transaction" => $external_id, "methodPay" => $payment_method, "id" => $external_id, "data" => $data, "type" => $type, "operator" => $operator, "price" => $price, "payment_name" => $payment_name
            ];
    ...
    if ($status == "pending") {
        ...
        $status = 1;
        $insertOST = mysqli_query($db_conn, "INSERT INTO `order_status_trackings`(`transaction_id`, `status_before`, `status_after`,`payment_method_before`, `payment_method_after`) VALUES ('$external_id', '$tStatus', '$status' ,'$tipe_bayar' ,'2')");

        $updateTrans = mysqli_query($db_conn, "UPDATE transaksi SET paid_date='$today11', tipe_bayar='2', shift_id='$lastShiftID', updated_at=NOW() WHERE id='$external_id'");
        if ($is_queue != 1 && $takeaway != 1 && $is_delivery != 1) {
            $updateTrans = mysqli_query($db_conn, "UPDATE transaksi SET status='$status', updated_at=NOW() WHERE id='$external_id'");
            if ($updateTrans) {
            ...
            if ($status == 1 || $status == '1') {
                ...
                $getIdMenu = mysqli_query(
                    $db_conn,
                    "SELECT dt.id_menu, dt.qty, dt.variant, m.is_recipe FROM `detail_transaksi` dt JOIN menu m ON m.id = dt.id_menu WHERE dt.id_transaksi='$external_id' AND dt.deleted_at IS NULL"
                );
                        ...
                        if ($arr_var != NULL) {
                            foreach ($arr_var as $vars) {
                                ...
                                foreach ($d_vars as $detail) {
                                    $var_id = $detail['id'];
                                    $ch = $fs->variant_stock_reduce($var_id, intval($qty));
                                }
                            }
                        }
        ...
        if ($UpdateCallback) {
            ...
            //     SET partner.saldo_ewallet=partner.saldo_ewallet+$amount
            //     WHERE transaksi.id='$external_id'"
            // );
        }
    } else {
        $status = 0;
        ...
        $insertOST = mysqli_query($db_conn, "INSERT INTO `order_status_trackings`(`transaction_id`, `status_before`, `status_after`,`payment_method_before`,`payment_method_after`) VALUES ('$external_id', '$tStatus', $status, '2' , '$payment_method_after')");
        ...
        $UpdateCallback = mysqli_query(
            $db_conn,
            "UPDATE `transaksi` SET `status_callback`='$data1',`callback_hit`=`callback_hit`+1, `callback_at`=NOW() WHERE id='$external_id'"
        );
    }
}
