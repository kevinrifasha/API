...
if(isset($tokenValidate['success']) && $tokenValidate['success']==0){
    ...
    if(isset($_GET['from'])&&!empty($_GET['from']) && isset($_GET['to'])&&!empty($_GET['to'])){
        ...
        $query = "SELECT t.id, pm.nama AS pmName, t.total, t.status, t.jam, t.tax, t.service, t.promo, t.diskon_spesial, t.employee_discount, t.point, t.no_meja AS noMeja, t.queue, t.charge_ur, t.program_discount,case when u.name is null or t.is_helper = 1 or t.is_pos = 1 then t.customer_name else u.name end AS uname FROM transaksi t LEFT JOIN users u ON u.phone=t.phone JOIN payment_method pm ON t.tipe_bayar=pm.id WHERE t.id_partner='$token->partnerID' AND DATE(t.jam) BETWEEN '$from' AND '$to' ORDER BY t.jam DESC";
        ...
        if (mysqli_num_rows($q) > 0) {
            ...
            foreach ($res as $f) {
                ...
                $tax = ceil(($subtotal+ $service + (int)$f['charge_ur'])*(double)$f['tax']/100);
