...
if (isset($tokenValidate['success']) && $tokenValidate['success'] == 0) {
    ...
    if (isset($_GET['transactionID']) && !empty($_GET['transactionID'])) {
        ...
        $query = "SELECT dt.id, dt.id_menu, dt.harga_satuan, dt.qty, dt.notes, dt.harga, dt.status, m.nama, dt.variant, c.name AS cName, c.id AS category_id, qty_delivered, CASE WHEN dt.qty-dt.qty_delivered>0 THEN 0 ELSE 1 END AS delivery_done, ss.nama AS serverName, dt.is_consignment FROM detail_transaksi dt JOIN menu m ON dt.id_menu=m.id JOIN categories c ON m.id_category=c.id JOIN employees ss ON dt.server_id=ss.id WHERE dt.id_transaksi='$transactionID' AND dt.deleted_at IS NULL ";
