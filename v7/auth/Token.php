...
class Token
{
  ...
  public function validate($tkn)
  {
    ...
    if (
      ...
      && isset($tknDecrypt->masterID) && isset($tknDecrypt->partnerID)  && isset($tknDecrypt->roleID)
    ) {
  ...
  public function reCreate($tkn)
  {
    ...
    if ($checker["status"] == 403) {
      ...
      $tkn = json_encode(['id' => $tknDecrypt->id, 'roleID' => $tknDecrypt->roleID, 'masterID' => $tknDecrypt->masterID, 'partnerID' => $tknDecrypt->partnerID, 'created_at' => $today, 'expired' => 5000]);
    }
